...
int cmdexec_CmdStartRunCmd (TCHAR *lpCmdStart, TCHAR *lpArgStart) {
    ...
    TCHAR *pszCommandLine = (*lpCmdStart == TEXT('\0')) ? NULL :
        (TCHAR *)cmdutil_Alloc (sizeof(TCHAR) * (5 + _tcslen (lpCmdStart) +
                                            _tcslen (lpArgStart)));
    ...
    if (pszCommandLine) {
        ...
        _tcscpy (pszCommandLine + 3, lpCmdStart);
        _tcscat (pszCommandLine, TEXT(" "));
        _tcscat (pszCommandLine, lpArgStart);
    }
...
void cmdexec_DirPrintFile (DirPrintHelperParms *pDPHP, WIN32_FIND_DATA *pWFD, int &iCurrentLine) {
    ...
    if (pDPHP->fOptions & CMD_DIR_WIDE) {
        ...
        if (pWFD->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            _stprintf (sSize, TEXT("%18s"), TEXT(" "));
        else
            _stprintf (sSize, TEXT("%18I64d"), liFileSize.QuadPart);

        TCHAR szDate[CMD_BUF_TINY];
        if (pDPHP->iDateType == 1)
            _stprintf (szDate, TEXT("%02d/%02d/%02d"), stm.wDay, stm.wMonth, stm.wYear % 100);
        else if (pDPHP->iDateType == 2)
            _stprintf (szDate, TEXT("%02d/%02d/%02d"), stm.wYear % 100, stm.wMonth, stm.wDay);
        else
            _stprintf (szDate, TEXT("%02d/%02d/%02d"), stm.wMonth, stm.wDay, stm.wYear % 100);

        TCHAR szTime[CMD_BUF_TINY];
        if (pDPHP->f24hr)
            _stprintf (szTime, TEXT("%02d:%02d "), stm.wHour, stm.wMinute);
        else{
      ...
      // check for 12am
          if(!hour) 
            ...
            _stprintf (szTime, TEXT("%02d:%02d%c"), hour,
...
int cmdexec_CmdDir (TCHAR *lpName, TCHAR *lpArgs) {
    ...
    if (! pplFiles) {
        TCHAR sBuffer[CMD_BUF_REG];
        _tcscpy (sBuffer, pCmdState->sCwd);
...
int cmdexec_CmdCd (TCHAR *lpName, TCHAR *lpArgs) {
    ...
    _tcscpy (pCmdState->sCwd, sBuffer);
...
{
    UINT uLen = 0;
    if (SUCCEEDED(UIntMult(pCmdState->iBufCmdChars, sizeof (TCHAR), &uLen))) {
        pCmdState->iBufCmdChars += CMD_BUF_INCREMENT;
...
static int cmdexec_FactorCommand (void) {
...
//
static int cmdexec_FindCommand (TCHAR *lpName) {
    for (int i = 0 ; i < CMD_COMMAND_NUMBER ; ++i) {
        ...
        if (! lpNameToken) {
            ...
            if (lpNameToken && (_tcsicmp(lpNameToken, TEXT(".exe")) == 0)) {
                ...
                if (nChar < pCmdState->iBufProcChars - 5) {
                    ...
                    _tcscpy (&pCmdState->lpBufProc[nChar], TEXT(".exe"));
                    break;
                }
            }
        }
