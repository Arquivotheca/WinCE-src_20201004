    ...
    )
{
    ...
    if (NULL != pDhcp) {
        ...
        pDhcp->AutoTimeout = DHCP_TIMEOUT_AUTOCFG;
...
STATUS GetDhcpConfig(DhcpInfo *pDhcp) {
    ...
    if (hKey) {
        ...
        if (GetRegDhcpConfigFlag(hKey, TEXT("AutoCfg"), pDhcp, DHCPCFG_AUTO_IP_ENABLED_FL, TRUE)) {
            ...
            GetRegDWORDValue(hKey, TEXT("AutoTimeout"), &pDhcp->AutoTimeout);
    ...
    )
{
        ...
        pCurrEvent = pNextEvent)
    {
        ...
        case DHCP_EVENT_RELEASE_LEASE:
            ...
            case DHCP_EVENT_PROCESS_T1:
            case DHCP_EVENT_PROCESS_T2:
                bDeleteCurrentEvent = TRUE;
                break;
            }
            break;
...
STATUS HandleProcessT1(DhcpInfo *pDhcp)
{
    DEBUGMSG(ZONE_TIMER, (L"DHCP:HandleProcessT1: pDhcp 0x%X %s\r\n", pDhcp, pDhcp->Name));
    CTEGetLock(&pDhcp->Lock, 0);
    PutInWorkerQ(pDhcp, DHCP_EVENT_PROCESS_T1);
    CTEFreeLock(&pDhcp->Lock, 0);
    return DHCP_SUCCESS;
}


STATUS HandleProcessT2(DhcpInfo *pDhcp)
{
    DEBUGMSG(ZONE_TIMER, (L"DHCP:HandleProcessT2: pDhcp 0x%X %s\r\n", pDhcp, pDhcp->Name));
    CTEGetLock(&pDhcp->Lock, 0);
    PutInWorkerQ(pDhcp, DHCP_EVENT_PROCESS_T2);
    CTEFreeLock(&pDhcp->Lock, 0);
    return DHCP_SUCCESS;
}


STATUS HandleProcessExpire(DhcpInfo *pDhcp)
{
    DEBUGMSG(ZONE_TIMER, (L"DHCP:HandleProcessExpire: pDhcp 0x%X %s\r\n", pDhcp, pDhcp->Name));
    CTEGetLock(&pDhcp->Lock, 0);
    PutInWorkerQ(pDhcp, DHCP_EVENT_PROCESS_EXPIRE);
    CTEFreeLock(&pDhcp->Lock, 0);
    return DHCP_SUCCESS;
}
...
void StartT1Timer(DhcpInfo *pDhcp) {
    ...
    CTEStartFTimer(&pDhcp->Timer, Ft, (CTEEventRtn)HandleProcessT1, pDhcp);    
    ...
    )
{
    ...
    do
    {
        ...
        Sock = (LPSOCK_INFO) CallAfd(Socket)(0x80000000 | AF_INET, SOCK_DGRAM, 
            IPPROTO_UDP, 0, NULL);
        if (Sock == NULL) {
            Status = GetLastError();
    ...
    if (FindDhcp(pDhcp, NULL)) {
        ...
        if ((DHCP_SUCCESS == Status) &&
            ...
            if (Status == DHCP_SUCCESS) {
                ...
                StartMidwayTimer(pDhcp, pDhcp->Lease, (CTEEventRtn)HandleProcessT2, 
                    (CTEEventRtn)HandleProcessExpire);
            }
            if (fStatus)
                ClearDHCPNTE(pDhcp);
            CloseDhcpSocket(pDhcp);
    ...
    if (FindDhcp(pDhcp, NULL)) {
        ...
        if ((DHCP_SUCCESS == Status) &&
            ...
            if (Status == DHCP_SUCCESS) {
                ...
                StartMidwayTimer(pDhcp, pDhcp->T2, (CTEEventRtn)HandleProcessT1,
                    (CTEEventRtn)HandleProcessT2);
            }
            if (fStatus)
                ClearDHCPNTE(pDhcp);
            CloseDhcpSocket(pDhcp);
    ...
    )
{
    ...
    case DHCP_EVENT_PROCESS_T1:         return L"DHCP_EVENT_PROCESS_T1";
    case DHCP_EVENT_PROCESS_T2:         return L"DHCP_EVENT_PROCESS_T2";
    case DHCP_EVENT_PROCESS_EXPIRE:     return L"DHCP_EVENT_PROCESS_EXPIRE";
    }
    ...
    )
{
    ...
    while (1) {
        ...
        while (!IsListEmpty(&pDhcp->EventList)) {
            ...
            case DHCP_EVENT_PROCESS_T1:
                ProcessT1(pEvent->pDhcp);
                break;

            case DHCP_EVENT_PROCESS_T2:
                ProcessT2(pEvent->pDhcp);
                break;

            case DHCP_EVENT_PROCESS_EXPIRE:
                ProcessExpire(pEvent->pDhcp);
                break;
            }
