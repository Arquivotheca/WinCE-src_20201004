	...
	int			*pcPkt)
{
    ...
    if (Flags & NEW_PKT_FL) {
        ...
        //
        if ((pDhcp->ClientIDType == DHCP_CLIENT_ID_TYPE_SOFT) && !(Flags & RENEW_PKT_FL)) {
...
//
{
    ...
    uint    NewT1,          fT1;
    uint    NewT2,          fT2;
    uint    NewDNS[MAX_RESOLVER],  fDNS;
    uint    NewWins[MAX_RESOLVER], fWins;
    uchar * pnew;
    ...
    if (cPkt >= (sizeof(*pPkt) - OPTIONS_LEN + 4) &&
		...
		do {
            ...
            while (p < pEnd) {
                ...
                //
                case DHCP_DNS_OP: 
                    memset(NewDNS, 0, sizeof(NewDNS));
                    if ((i = p[1]) > sizeof(NewDNS)) {
                        i = sizeof(NewDNS);
                    }
                    pnew = (uchar *)NewDNS;
                    fDNS++;
                ...
                case DHCP_NBT_SRVR_OP:
                    memset(NewWins, 0, sizeof(NewWins));
                    if ((i = p[1]) > sizeof(NewWins)) {
                        i = sizeof(NewWins);
                    }
                    pnew = (uchar *)NewWins;
                    fWins++;
		...
		do {
            ...
            while (p < pEnd) {
                ...
                case DHCP_ROUTER_OP:
                    ...
                    break;
                case DHCP_DNS_OP:
                    memcpy(pDhcp->DNS, NewDNS, sizeof(NewDNS));
                    break;
                case DHCP_DOMAIN_NAME_OP:
                    ...
                    break;
                case DHCP_NBT_SRVR_OP:
                    memcpy(pDhcp->WinsServer, NewWins, sizeof(NewWins));
                    break;
                case DHCP_IP_LEASE_OP:
