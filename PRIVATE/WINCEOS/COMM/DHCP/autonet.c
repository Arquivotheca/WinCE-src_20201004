...
extern DhcpInfo *FindLockRefDhcp(DhcpInfo *pDhcp, PTSTR pName);  // dhcp.c
...
#define ADDRESS_STRING_LENGTH 32
    ...
    )
{
    ...
    BOOL bRet;
#ifdef DEBUG
    TCHAR Addr[ADDRESS_STRING_LENGTH];
    TCHAR Reply[ADDRESS_STRING_LENGTH];
#endif
    ...
    if (EchoReplies->Address != dwIPAddr) {
        DEBUGMSG(ZONE_AUTOIP, (L"DHCP:PingDhcp: Echo IPAddr(%s) != Request IPAddr(%s)\n",
            AddrToString(EchoReplies->Status, Reply, ADDRESS_STRING_LENGTH),
            AddrToString(dwIPAddr, Addr, ADDRESS_STRING_LENGTH)));
        goto pd_exit;
    }
    ...
    )
{
...
#ifdef DEBUG
    TCHAR Addr[ADDRESS_STRING_LENGTH];
    TCHAR Gateway[ADDRESS_STRING_LENGTH];
#endif
    ...
    if ((pDhcp->IPAddr == 0) || (pDhcp->Gateway == 0)) {
        DEBUGMSG(ZONE_FUNCTION|ZONE_AUTOIP, (L"DHCP:-CouldPingGateway: IPAddr = %s, Gateway = %s\n",
            AddrToString(pDhcp->IPAddr, Addr, ADDRESS_STRING_LENGTH),
            AddrToString(pDhcp->Gateway, Gateway, ADDRESS_STRING_LENGTH)));
        return DHCP_FAILURE;
    }
    ...
    )
{
    ...
    if (FindLockRefDhcp(pDhcp, NULL)) {
        if (SetAutoCfgDHCPNTE(pDhcp)) {
            if (DHCP_SUCCESS == (Status = DhcpInitSock(pDhcp, 0))) {
                ...
                if (DHCP_SUCCESS == Status) {
                    ...
                    pDhcp->IPAddr = IPAddr;
                    TakeNetDown(pDhcp, RETAIN_IPADDR);  // Retain IPAddr from the DHCPOFFER
        ...
        DEBUGMSG(ZONE_AUTOIP, (L"DHCP:ProcessAutoIP - FindLockRefDhcp failed!\n"));
    }
    DEBUGMSG(ZONE_FUNCTION|ZONE_AUTOIP, (L"DHCP:-ProcessAutoIP\n"));
    return;
    ...
    )
{
    ...
    do {
        ...
        DEBUGMSG(ZONE_AUTOIP, (L"DHCP:AutoIP:Trying autoconfig address: %s\n",
            AddrToString(AttemptedAddress, Addr, ADDRESS_STRING_LENGTH)));
        ...
        if (SetAddrResult == IP_PENDING)
        {
            ...
            // BUGBUG: Should we fail if ARPEvent does not get set and we timeout?
        ...
        if (SetAddrResult == IP_SUCCESS)
        {
            if (ERROR_SUCCESS == pDhcp->ARPResult)
            {
                ...
                (*pfnAfdAddInterface)(pDhcp->Name, pDhcp->Nte, 
