...
IrlmpLinkControlReq(IRDA_MSG *pMsg)
{
         ...
         pIrdaLinkCb = pIrdaLinkCbNext)
    {
        ...
        if (rc != SUCCESS)
        {
            ...
            // BUGBUG: this will leave some links up with resources!! Although
            //         we probably only support one link for now anyways.
            break; 
        }
    }
                     ...
                     BOOLEAN LocallyGenerated)
{
    ...
    // BUGBUG WHACK FOR IRCOMM YUK YUK !!
...
{
    if (pIrlmpCb->LinkState == LINK_CONNECTING) {
...
// I'm out of time, no more function hdrs
            ...
            memcmp(pClassName, pObject->pClassName, ClassNameLen) == 0)
        {
            ...
            while (pAttrib != NULL)
            {
                    ...
                    memcmp(pAttrib->pAttribName, pAttribName, AttribNameLen) == 0)
                {
                    ...
                    PREFAST_SUPPRESS(394, "Needs to be done here");
