              ...
              PTDI_CONNECTION_INFORMATION   pRetAddr)
{
    ...
    DWORD             dwTemp;	// For compiler bug work around
...
RetryIASQuery:
        ...
        {
			// BUGBUG when does this get retried?
			DEBUGMSG (ZONE_ERROR,
        ...
        else    
        {
            switch(IASQueryStatus)
            {
              case IRLMP_MAC_MEDIA_BUSY:
				// BUGBUG, changed it to retry the query  
				DEBUGMSG (1, (TEXT("GOT IRLMP_MAC_MEDIA_BUSY in TDI:IRLMPConnect, retrying query\r\n")));
                *(volatile DWORD*)&dwTemp = 0;	// This avoids an SH3 compiler bug (#14740).
                goto RetryIASQuery;
                break;
                
              case IRLMP_IRLAP_REMOTE_DISCOVERY_IN_PROGRESS:
                /* wmz retry the query after DISCOVERY_IND */
				// BUGBUG, changed it to wait for a discovery indication and retry the query
    ...
    )
{
    ...
    if (pConn->ConnState == IRLMP_CONN_OPEN)
    {
        if (pConn->LingerTime <= LINGER_TIMEOUT) {
            ...
            DEBUGMSG (0, (TEXT("IrDA:: LingerTimerFunc(): LastSendsRemaining <- 0\r\n")));                
            bDisconnected = TRUE;
        } else {
        ...
        if (pConn->LastSendsRemaining <= pConn->SendsRemaining) {
            ...
            //
            DEBUGMSG (0, (TEXT("IrDA:: LingerTimerFunc(): !! LastSendsRemaining[%d] - SendsRemaining[%d]. \r\n"),
            ...
            DEBUGMSG (0, (TEXT("IrDA:: LingerTimerFunc(): LastSendsRemaining[%d] SendRemaining[%d] \r\n"),
                pConn->LastSendsRemaining,
                pConn->SendsRemaining));                          
        }
    }
    ...
    if (bDisconnected) {
        TDI_STATUS  TdiStatusToIndicate;
        ...
        //  Abortive close if there's pending data to be retrieved.
        //  Otherwise, no data sent and no data to be indicated, indicate a graceful disconnect.

        if (pConn->NotIndicated == 0 && pConn->IndicatedNotAccepted == 0)
            TdiStatusToIndicate = TDI_DISCONNECT_WAIT;
        else
            TdiStatusToIndicate = TDI_DISCONNECT_ABORT;
        
        pAddrObj->pEventDisconnect(
            pAddrObj->pEventDisconnectContext,
            pConn->pConnectionContext, 0, NULL, 
            0, NULL, TdiStatusToIndicate);
                 ...
                 PTDI_CONNECTION_INFORMATION pRetInfo)
{
    ...
    case IRLMP_CONN_OPEN:
        if (Flags != TDI_DISCONNECT_ABORT) {
            pConn->LastSendsRemaining = pConn->SendsRemaining;
            DEBUGMSG (0, (TEXT("IrDA:: IRLMP_Disconnect(): LastSendsRemaining[%d] SendRemaining[%d] \r\n"),
                pConn->LastSendsRemaining,
                pConn->SendsRemaining));              
              ...
              PNDIS_BUFFER pNDISBuf)
{
    ...
    if (! IsListEmpty(&pConn->RecvBuffList))
    {
        ...
        while (SpaceLeft > 0 && ! IsListEmpty(&pConn->RecvBuffList))
        {
            ...
            CeSafeCopyMemory(((BYTE *) VirtualAddress) + BufferLength,
                   pRecvBuff->pRead, 
                   min(pRecvBuff->DataLen, SpaceLeft));
           ...
           PNDIS_BUFFER pNDISBuf)
{
    ...
    switch(pConn->ConnState)
    {
        case IRLMP_CONN_OPEN:
            pConn->SendsRemaining++;
            DEBUGMSG (0, (TEXT("IrDA:: IRLMP_Send(): LastSendsRemaining[%d] SendRemaining[%d] \r\n"),
                pConn->LastSendsRemaining,
                pConn->SendsRemaining));                          
            FREE_CONN_LOCK(pConn);
