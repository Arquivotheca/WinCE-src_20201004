...
/*------------------------------------------------------------------------------
    ...
    This function gets the necessary SEID based on INT/ACP roles.  This
    function will determine this based on fInitiator param and who actually
    initiated the stream.
    
    Parameters:
        pStream:    Stream object
        fInitiator: TRUE if we are looking for SEID for an INT command
    
    Returns (BYTE): The INT or ACP SEID
------------------------------------------------------------------------------*/
inline BYTE GetSEID(Stream* pStream, BOOL fInitiator)
{
    // If current command/response role (fInitiator) is equal to the stream's
    // role then we are in the normal case where we are checking the ACP SEID.
    // If the roles are not equal the ACP is sending a command to the INT in
    // which case must check the INT SEID.
    return (pStream->fInitiator == fInitiator) ? pStream->bAcpSeid : pStream->bIntSeid;   
}
...
/*------------------------------------------------------------------------------
    ...
    Returns (AVDT_NS.Stream): Pointer to a stream struct
------------------------------------------------------------------------------*/
Stream* FindStream(BYTE bSEID, BOOL fInitiator)
{
    SVSUTIL_ASSERT(avdtp_IsLocked());
    ...
    for (StreamList::iterator it = gpAVDTP->listStreams.begin(), itEnd = gpAVDTP->listStreams.end(); it != itEnd; ++it) {
        BYTE bStreamSeid = GetSEID(*it, fInitiator);
        if (bStreamSeid == bSEID) {
            pStream = *it;
            break;
        }
    }
...
------------------------------------------------------------------------------*/
Stream* NewStream(BYTE bAcpSeid, HANDLE hOwner, HANDLE hSessSignal)
{
    SVSUTIL_ASSERT(avdtp_IsLocked());

    Stream* pStream = new Stream(bAcpSeid, hOwner, hSessSignal);
    if (! pStream) {
...
/*------------------------------------------------------------------------------
    ...
    Parameters:
        pStream:            Stream object
        fCloseImmediately:  Close all sessions immediately (no timeout)
        iError:             Reason for closing the stream
------------------------------------------------------------------------------*/
int CloseStream(Stream* pStream, BOOL fCloseImmediately, int iError)
{
    ...
    // Close or timeout the signaling session
    if (VerifyStream(hStream) && pStream->hSessSignal) {
        Session* pSession = VerifySession(pStream->hSessSignal);
        if (pSession) {
            pStream->hSessSignal = 0; // clear this before trying to find a different stream
            if (! FindStream(pSession->hContext)) {
                // No other stream is using this session, go ahead and
                // close it or time it out.
                
                EnsureActiveMode(pStream, FALSE);
                
                if (fCloseImmediately) {
                    CloseSession(pSession, iError);
                } else {
...
DWORD WINAPI TimeoutStream(LPVOID pArg)
{
    ...
    if (pStream) {
        ...
        CloseStream(pStream, FALSE, ERROR_TIMEOUT);
    }
                ...
                BYTE bError)
{
    ...
    // Clear the session timer and reset it if there are no existing streams using
    // this session.
    ClearTimeout(pSession);
    if (! FindStream(pSession->hContext))
    {
                ...
                BYTE bAcpSEID)
{
    ...
    // TODO: Should we make sure a stream doesn't already exist?  Is that possible?

    Stream* pStream = NewStream(bAcpSEID, pOwner->hContext, pSess->hContext);
    if (! pStream) {
                ...
                BYTE bError)
{
    ...
    if (! bError) {
        ...
        ClearTimeout(pStream);
        // Ensure underlying session timeout is cleared.  This may not be the case if
        // the remote device skips a get_capabilities.
        ClearTimeout(pSession);
    }
                ...
                BYTE bError)
{
    ...
    if (iRes == ERROR_SUCCESS) {
        ...
        if (pStream && !bError) {
            ...
            // The INT of the close request will close the media session.  Don't do any clean
            // up here immediately.  Wait for a disconnect on the media channel then we will 
            // clean up the stream and timeout the signaling channel.  In case peer is broken
            // let's timeout the stream.
            ScheduleTimeout(pStream, gpAVDTP->cIdleTimeoutSec);
        }
    }
                ...
                BYTE bError)
{
    ...
    if (iRes == ERROR_SUCCESS) {
        ...
        if (pStream && !bError) {
            ...
            // The INT of the abort request will close the media session.  Don't do any clean
            // up here immediately.  Wait for a disconnect on the media channel then we will 
            // clean up the stream and timeout the signaling channel.  In case peer is broken
            // let's timeout the stream.
            ScheduleTimeout(pStream, gpAVDTP->cIdleTimeoutSec);
                ...
                BYTE bAcpSEID)
{
    ...
    Stream* pStream = NewStream(bAcpSEID, pOwner->hContext, pSession->hContext);
    if (! pStream) {
            ...
            PBYTE pbFailedCategory)
{
    ...
    *pbFailedCategory = 0;
    ...
    AVDTP_CONTEXT* pOwner = FindOwner(bAcpSEID);
    if (! pOwner) {
        IFDBG(DebugOut (DEBUG_ERROR, L"[AVDTP] SetConfigurationRequestUp : Could not find owner context for SEID %d - ERROR_NOT_FOUND\n", bAcpSEID));
    ...
    Stream* pStream = FindStream(bAcpSEID, FALSE);
    if (! pStream) {
        IFDBG(DebugOut (DEBUG_ERROR, L"[AVDTP] SetConfigurationRequestUp : ERROR_NOT_FOUND\n"));
        *pbFailedCategory = pConfig[0].bServiceCategory; // On error, pick first category per test spec
        return AVDTP_ERROR_BAD_SEID;
    }

    if (pStream->state.IsConfigured()) {
        IFDBG(DebugOut (DEBUG_ERROR, L"[AVDTP] SetConfigurationRequestUp : ERROR_INVALID_STATE\n"));
        *pbFailedCategory = pConfig[0].bServiceCategory; // On error, pick first category per test spec
            ...
            BYTE bError)
{
    ...
    if (! bError) {
        ...
        CloseStream(pStream, FALSE, ERROR_GRACEFUL_DISCONNECT);
    }
...
BYTE SuspendRequestUp(
            ...
            DWORD cbSEIDList)
{
    IFDBG(DebugOut(DEBUG_AVDTP_TRACE, L"++SuspendRequestUp: ba:%04x%08x transaction:%d\n", 
        pSession->ba.NAP, pSession->ba.SAP, bTransaction));    
