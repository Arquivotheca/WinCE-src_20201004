...
static int tdir_data_up_ind (void *pUserContext, HANDLE hConnection, BD_BUFFER *pba, int additional_credits) {
    ...
    while (gpTDIR->fIsConnected && (cBytes > 0) && pConn && pConn->pRecvList && pConn->pRecvList->pNdisBuffer) {
        ...
        IFDBG(DebugOut (DEBUG_TDI_TRACE, L"tdir_data_up_ind 0x%08x :: satisfied read request for %d bytes\n", pConn, iNdisUsed));
    ...
    if (gpTDIR->fIsConnected && (cBytes > 0) && pConn && pConn->pAddr &&
        ...
        //
        // First indicate up the received data
        //
        if (pConn->pAddr && (0 == pConn->cbDataIndicated)) {
            ...
            TDI_STATUS TdiStatus = pEventReceive (
            ...
            IFDBG(DebugOut (DEBUG_TDI_TRACE, L"tdir_data_up_ind 0x%08x :: indicated %d bytes\n", pConn, cBytes));

            if (TDI_SUCCESS == TdiStatus) {
                SVSUTIL_ASSERT(BytesTaken <= cBytes);
                pba->cStart += BytesTaken;
                cBytes = BufferTotal(pba);
            }
            ...
            IFDBG(DebugOut (DEBUG_TDI_TRACE, L"tdir_data_up_ind 0x%08x :: data already indicated, stored %d bytes\n", pConn, cBytes));
        }

        //
        // Buffer any received data not taken by TDI
        //
        if (cBytes) {
            ...
            if (pRecv) {
                ...
                if (pConn && (pConn->irecvd > pConn->xon_lim) && (! pConn->fc_local) && (! pConn->fc_credit)) {
                    IFDBG(DebugOut (DEBUG_TDI_TRACE, L"Switching flow off for connection 0x%08x\n", pConn));
...
PTDI_CONNECTION_INFORMATION        pRetAddr
    ...
    if (pC) {
        ...
        // or existing connection.  The channel may not always be zero here since the
        // client can retry the connect() call and it will be non-zero since we set it
        // at the bottom of this function.
        SVSUTIL_ASSERT ((pC->pAddr->b.NAP == 0) && (pC->pAddr->b.SAP == 0)); // Wildcard
        ...
        if (pConn->pRecvList && pConn->pRecvList->pBuffer) {
            ...
            if (cDataLeft && (pConn->cbDataIndicated == 0)) {
                ...
                TDI_STATUS TdiStatus2 = pEventReceive (pEventReceiveContext, pConnectionContext,
                    0, 0, cDataLeft, (unsigned int *)&BytesTaken, NULL, &EventRecvCompleteInfo);
    ...
    CRITICAL_SECTION    *pAfdCS)
{
    ...
    //
    // Register the stack driver with AFD
    //
            ...
            while (pConn) {
                ...
                while (pRecvList) {
                    if (pRecvList->pNdisBuffer) {
                        ...
                        DebugOut (DEBUG_OUTPUT, L"    RECV 0x%08x NDIS 0x%08x Size %d Used %d Ctx 0x%08x cb 0x%08x\n",
                            pRecvList, pRecvList->pNdisBuffer, iSize, pRecvList->iNdisUsed, pRecvList->pRecvContext, pRecvList->pRecvEvent);
                    } else
