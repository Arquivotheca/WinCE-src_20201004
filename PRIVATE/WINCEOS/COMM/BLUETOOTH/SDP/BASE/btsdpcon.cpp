...
SdpConnection::~SdpConnection()
{
    ...
    //
    // BUGBUG:  clean up the irp queue here?
    //
    if (pReadBuffer) {
    ...
    )
{
    ...
    if (removed == FALSE) {
        ...
        //
        // BUGBUG:  need to do this once we have disconnected
        //
        // pCon->btAddress = 0x0;
...
void SdpConnection::ReadWorkItemWorker()
{
    if (GetChannelState() == SdpChannelStateClosed) {
        ...
        if (IsClient()) {
            ...
            //
            // if there is an error, we must do something more intelligent here
            // based on the specific status value
            //
            // BUGBUG:  for now, assume that the server is going down so release
            //          our ref to the connection
            //
            Release(pReadIrp);
        }
    }
    ...
    )
{
    ...
    if (bytesSoFar == 0) {
        ...
        //
        // BUGBUG:  this requires that we have at most 5 bytes (at least 2) in the
        //          first response.  This needs to be fixed!
        //
        SdpRetrieveHeader(pAttribList, type, sizeIndex);
...
BOOLEAN SdpConnection::Disconnect(SdpChannelState *pPreviousState)
{
    ...
    if (disconnect) {
        ...
        if (IsClient()) {
            ...
            //
            // BUGBUG:  need to clean up client requests here?  Also, clean up
            //          file objects as well.
            //
        }
    }
