...
//
{
	...
	{
		pGetDevCaps = NULL;
		if (SUCCEEDED(ULongAdd(sizeof(NDIS_TAPI_GET_DEV_CAPS), dwExtraSize, &dwGetDevCapsSize)))
			pGetDevCaps = LocalAlloc(LPTR, dwGetDevCapsSize);

		DEBUGMSG(ZONE_ERROR && NULL == pGetDevCaps, (L"PPP: ERROR - Unable to allocate NDIS_TAPI_GET_DEV_CAPS\n"));
                            ...
                            LPVARSTRING      pDeviceConfigOut)
{
	...
	{
		PREFAST_SUPPRESS(12011, "The following addition cannot overflow because of the check above.");
