...
//
{
	...
	PREFAST_ASSERT(wcslen(pRasDialParams->szDomain) <= DNLEN);
	PREFAST_ASSERT(wcslen(pRasDialParams->szUserName) <= UNLEN);

	wszDomain = L"";
	wszSeparator = L"";
	if (pRasDialParams->szDomain[0])
	{
		wszDomain = pRasDialParams->szDomain;
		wszSeparator = L"\\";
	}

	StringCchPrintfW(wszUserName, DNLEN + 1 + UNLEN + 1, L"%s%s%s", wszDomain, wszSeparator, pRasDialParams->szUserName);
...
DWORD APIENTRY
AfdRasEnumEntries (IN     LPTSTR          reserved, 
                   IN     LPTSTR          lpszPhoneBookPath, 
