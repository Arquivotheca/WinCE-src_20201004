...
public:
    ...
    inline LONG WaitForThreads (LONG RefCount)
    {
        ...
        while (RefCount & VOLUME_REFERENCE_THREAD_MASK) {
            ...
            // shouldn't happen, but we don't want to deadlock due to a .
            //
    ...
    LRESULT InstallFilter (const WCHAR* RootKey, const WCHAR* SubKey);

    // Build a list of filters to associate with this volume and add each one
    // to the filter stack using FileSystemFilterDriver_t.Hook. Each time a filter
    // is loaded, mark the registry key accordingly so we will not load it on
    // subsequent calls to InstallFilters. Calling mulitple times should only
    // hook new filters into the chain. This function may be exposed using a new
    // volume FSCTL to allow new filters to be added at run time.
    LRESULT InstallFilters ();
...
protected:
    ...
    inline LONG RemoveReference (LONG RefType)
    {
        ...
        //Dont consider power-down bit for tear down decision.
        if (0 == (RefCount & ~VOLUME_REFERENCE_POWERDOWN)) {
            //No other reference; so destroy (or tear down) the volume.
            Destroy ();           
        }
