    ...
    )
{
    ...
    if (piml)
    {
        ...
        else
        {
            ...
            // Check for int under/overflow and try to realloc
            unsigned int initialCount = 0;
            unsigned int newIncrement = 0;
            if (FAILED(IntToUInt(cInitial, &initialCount)) ||
                FAILED(UIntAdd(initialCount, 1, &newIncrement)) ||
                !piml->ReAllocBitmaps(newIncrement))
            {
                // If we fail, try to realloc for one more
                if (!piml->ReAllocBitmaps(1))
                {
                    goto Error;
                }
            }
...
// The correspondoing Win32 API takes an overloaded parameter which can either be a string or an ResourceID. 
// When thunking into kernel, kernel's stringent check based on the datatype of the parameter causes 
// an exception when a valid ResourceID is sent. For this reason GWE entry point for the API expects coredll to
// expand the parameters allowing cleaner parameter checking.
...
LoadImageW_I(
    ...
    const   WCHAR*    pImageName,   // name of image
            int                cx,         
            int                cGrow,
            COLORREF    crMask,
            unsigned int  uType,
    ...
    )
{
    ...
    hbmImage = (HBITMAP)::LoadImageW_I(hInstance, ImageNameResourceID, pImageName, uType, 0, 0, uFlags);
...
{	
    ...
    // NOTE: Memory for image control object is allocated in the
    // shared heap. This check needs to be modified if the
    // implementation for image control handles is changed.
    if (!GweMemoryVoid_t::IsSharedMemory(this))
        {
        goto leave;
        }                                                 
	
	if (m_Sig != IMAGELIST_SIG)
		{
		goto leave;
		}
