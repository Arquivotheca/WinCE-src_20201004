...
BOOL HdstubRegisterClient (HDSTUB_CLIENT *pClient, int Disposition)
{
    ...
    switch (Disposition)
    {
        ...
        case HdstubClientWatson:
            DEBUGGERMSG (HDZONE_CLIENT, (L"  HdstubRegisterClient: Register watson client.\r\n"));
            pClientWatson = pClient;
            // Fall through to add to end of client list
        ...
        DWORD dwExceptionPhase)
{
    ...
    DEBUGGERMSG(HDZONE_ENTRY, 
        (TEXT("++HdstubTrapException: pex = 0x%.8x, pContext = 0x%.8x, dwExceptionPhase = %d, ExcAddr=0x%08X\r\n"),
        pex, pContext, dwExceptionPhase, pex ? pex->ExceptionAddress : NULL));
    ...
    if (HD_EXCEPTION_PHASE_DUMPONLY == dwExceptionPhase)
    {
        // This is a request to generate a Watson dump, so hand directly to any registered Watson client
        // as a second-chance exception and short-circuit the rest of the dispatch.
        if (NULL != pClientWatson)
        {
            pClientWatson->pfnException(pex, pContext, TRUE);
        }
        return FALSE;
    }
    else if (HD_EXCEPTION_PHASE_SECONDCHANCE == dwExceptionPhase)
    {
        b2ndChance = TRUE;
    }
    else
