...
HLOCAL WINAPI LocalFree_stub(HLOCAL hMem)
{
    ...
    if (hRet) {
        ...
        while (index++ < 5) {
            if (bVal = TryEnterCriticalSection (g_lpcsHeapList))
                break;
            Sleep(10);
        }
...
void DllHeapInit(void)
{
    PMODULEHEAPINFO pInfo = NULL;
    
    // get the dll heap entry structure from coredll
    if (pInfo = (PMODULEHEAPINFO) CeGetModuleHeapInfo()) {        
        ...
        if (g_hDllHeap || (g_hDllHeap = HeapCreate(0,0,0))) {
