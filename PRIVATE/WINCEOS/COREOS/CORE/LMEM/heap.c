...
HANDLE WINAPI Int_HeapCreate(DWORD flOptions, DWORD dwInitialSize, DWORD dwMaximumSize)
{
    ...
    return DoHeapCreate (flOptions, dwInitialSize, dwMaximumSize, AllocMemVirt, FreeMem);
}
...
LPVOID WINAPI Int_HeapReAlloc(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem, DWORD dwBytes)
{
    ...
    if (cbItem & 1) {
        ...
        //BUGBUG: need to handle "in place" realloc here!
    ...
    if (!pRet && !(dwFlags & HEAP_REALLOC_IN_PLACE_ONLY)) {
