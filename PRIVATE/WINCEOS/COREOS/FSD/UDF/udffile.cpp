...
//
BOOL CFile::Initialize( CVolume* pVolume, NSRLBA Location )
{
    BOOL fResult = TRUE;
    LRESULT lResult = ERROR_SUCCESS;
                                  ...
                                  CFileInfo* pFileInfo )
{
    ...
    if( !pFile )
    {
        ...
        //
        if( !pFile->Initialize( m_pVolume, Fid.Icb.Start ) )
        {
            lResult = GetLastError();
            goto exit_findnextfile;
        }
...
//
BOOL CDirectory::Initialize( CVolume* pVolume, NSRLBA Location )
{
    BOOL fResult = CFile::Initialize( pVolume, Location );
    LRESULT lResult = ERROR_SUCCESS;
    Uint64 FileOffset = 0;
    ...
    while( RemainingBytes )
    {
        ...
        while( ExtentInfo.Length.Length )
        {
            ...
            // we should not be here as this will cause aninfinite loop here, break out, may be a bad Disk 
...
BOOL CStream::InitCreationFromEAs( PICBFILE pIcb )
{
    ...
    if( pIcb->IcbEA.Length.Length )
    {
        ...
        if( !pEAFile->Initialize( m_pVolume, pIcb->IcbEA.Start ) )
        {
            fResult = FALSE;
            goto exit_initcreationfromeas;
        }
