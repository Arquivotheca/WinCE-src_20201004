			  ...
			  PDWORD pdwActualOut)
{
	...
	case MQAPI_CODE_MQReceiveMessage:
		{
			...
			MAP_RET_SET_LAST_ERR(pProvVar,pdm->pMsgProps,SCPROPVAR);
			MAP_RET_SET_LAST_ERR(pOverlapped,pdm->lpOverlapped,OVERLAPPED);
		
			hr = scapi_MQReceiveMessage (pdm->hQueue, pdm->dwTimeout, pdm->dwAction,
				pProvVar,pOverlapped,pdm->pfnReceiveCallback,
	...
	case MQAPI_CODE_MQLocateBegin:
		{
			HANDLE hCallerProc = GetCallerProcess ();
			MQSORTSET *pMqSortset;
			HANDLE    *phEnum;

			MAP_RET_SET_LAST_ERR(pMqSortset,(LPVOID)dwLenIn,MQSORTSET);
			MAP_RET_SET_LAST_ERR(phEnum,(LPVOID)dwLenOut,HANDLE);
	...
	case MQAPI_CODE_MQMgmtGetInfo2:
		{
			...
			MAP_RET_SET_LAST_ERR(pPropVar,(LPVOID)dwLenOut,PROPVARIANT);

			hr = scmgmt_MQMgmtGetInfo2 ((LPCWSTR)pBufIn, (LPCWSTR)pBufOut, (DWORD)dwLenIn,
							(PROPID *)pdwActualOut, pPropVar);
		}
		break;
