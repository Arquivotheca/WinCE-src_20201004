...
void CreateNewServiceSocket(SOCKET sockNew, fsdev_t *pOwner, SOCKADDR_STORAGE *pSockAddr,
                            DWORD cbSockAddr, int iProtocol,WCHAR *szRegWritePath) {
	ServiceSocket *pServSocket;

	pServSocket = (ServiceSocket*) svsutil_GetFixed(g_pServiceSocketsMemDescr);
...
BOOL ServiceAddPortInternal(HANDLE hService, SOCKADDR *pSockAddr, int cbSockAddr, int iProtocol, WCHAR *szRegWritePath, BOOL fWriteRegistry) {
	...
	CreateNewServiceSocket(sockNew,pOwner,(SOCKADDR_STORAGE*)pSockAddr,cbSockAddr,iProtocol,szStoreRegWritePath);
...
BOOL SuperServicesInit(void) {
	...
	if (NULL == (hIphlpLib = LoadLibrary(L"iphlpapi.dll"))) {
...
DWORD WINAPI ServicesNotifyAddrChangeThread(LPVOID lpv) {
	...
	while (1) {
		...
		if (dw == WAIT_FAILED) {
			...
			DEBUGCHK(dw - WAIT_OBJECT_0 < dwNumEvents);
