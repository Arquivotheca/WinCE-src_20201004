...
typedef struct fsdev_t {
	...
	DWORD dwFlags;
	WCHAR szFriendlyName[FRIENDLY_NAME_LEN];
} fsdev_t;
...
HANDLE RegisterServiceEx( LPCWSTR lpszType, DWORD dwIndex, LPCWSTR lpszLib, DWORD dwInfo, DWORD dwFlags, LPCWSTR lpszFullRegPath, LPCWSTR lpszFriendlyName);
...
void FreeService(fsdev_t *lpdev, BOOL fRemoveFromDyingList=TRUE, BOOL fSpinThread=TRUE, BOOL fLogEvent=TRUE);
...
// Friendly 
#define     SERVICE_FRIENDLY_NAME         L"FriendlyName"
