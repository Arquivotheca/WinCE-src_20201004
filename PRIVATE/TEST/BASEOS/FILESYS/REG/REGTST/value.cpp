...
TESTPROCAPI Tst_RegDeleteVal_Stress(UINT uMsg, TPPARAM tpParam, LPFUNCTION_TABLE_ENTRY lpFTE)
{
	...
	DWORD cbData=2*1024;
	DWORD dwType=0;
	DWORD dwMemSize_Begin=0;
	DWORD dwMemSize_End=0;
...
TESTPROCAPI Tst_RegQueryValue_ValName(UINT uMsg, TPPARAM tpParam, LPFUNCTION_TABLE_ENTRY lpFTE)
{
	...
	if (lRet=RegSetValueEx(hKeyNew, _T(""), 0, REG_SZ, (PBYTE)rgszValName, (_tcslen(rgszValName)+1)*sizeof(TCHAR)))
	{
	...
	//
	if (lRet=RegDeleteValue(hKeyNew, _T("")))
	{
	...
	_stprintf(rgszValName, _T("VALUE_%d"), 0);
	rgszValName[_tcslen(rgszValName)] = _T('A');
	if (ERROR_SUCCESS == (lRet=RegQueryValueEx(hKeyNew, rgszValName, NULL, &dwType, pbData, &cbData)))
	{
