...
TESTPROCAPI tst_IOCTL_DISK_SETINFO(UINT uMsg,TPPARAM tpParam, 
LPFUNCTION_TABLE_ENTRY lpFTE )
{
	...
	DRIVER_TEST *pMyTest;
	pMyTest = new IOCTL_DISK_SETINFO_TEST(g_hDisk,g_pKato,g_storageDeviceInfo.dwDeviceType);
	if(!pMyTest)
	{
		g_pKato->Log(LOG_FAIL,_T("FAILED to allocate a new IOCTL_DISK_SETINFO_TEST class.\n"));
		bRet = FALSE;
		goto EXIT;
	}
...
BOOL IOCTL_DISK_SETINFO_TEST::CustomFill(void)
{
	BOOL bRet = Fill();
	return bRet;
}

BOOL IOCTL_DISK_SETINFO_TEST::LoadCustomParameters(void)
{
...
BOOL IOCTL_DISK_SETINFO_TEST::RunIOCTL(void)
{
	try
	{
...
BOOL IOCTL_DISK_SETINFO_TEST::ATAPIValidate(void)
{
	BOOL retVal = TRUE;
...
BOOL IOCTL_DISK_SETINFO_TEST::Validate(void)
{
