...
TESTPROCAPI tst_DISK_IOCTL_READ(UINT uMsg,TPPARAM tpParam, 
LPFUNCTION_TABLE_ENTRY lpFTE )
{
	...
	DRIVER_TEST *pMyTest;
	pMyTest = new DISK_IOCTL_READ_TEST(g_hDisk,g_pKato,g_storageDeviceInfo.dwDeviceType);
	if(!pMyTest)
	{
		g_pKato->Log(LOG_FAIL,TEXT("FAILED to allocate a new DISK_IOCTL_READ_TEST class.\n"));
		return FALSE;
	}
...
BOOL DISK_IOCTL_READ_TEST::CustomCleanup()
{
	//cleanup
	if(m_dataBuffer)
	{
...
BOOL DISK_IOCTL_READ_TEST::CustomFill(void)
{
...
BOOL DISK_IOCTL_READ_TEST::LoadCustomParameters(void)
{
...
BOOL DISK_IOCTL_READ_TEST::RunIOCTL(void)
{
	BOOL retVal = TRUE;
	try
	{
		...
		{
			//nothing to do here, since we can't write to CDROM
			g_pKato->Log(LOG_DETAIL, _T("SKIPPING Write since we can't write to CD or DVD media."));
		}
		else
		{
...
BOOL DISK_IOCTL_READ_TEST::ATAPIValidate(void)
{
	BOOL retVal = TRUE;
...
BOOL DISK_IOCTL_READ_TEST::Validate(void)
{
