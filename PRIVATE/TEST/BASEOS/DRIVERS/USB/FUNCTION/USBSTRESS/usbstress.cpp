 ...
 */
extern "C" BOOL USBInstallDriver(LPCWSTR szDriverLibFile)   // @parm [IN] - Contains client driver DLL name
{
    USBMSG(LOG_DETAIL, _T("Start USBInstallDriver %s "), szDriverLibFile);
    ...
    if(usbDriver.IsOk())
    {
        ...
        ASSERT(fRet);
        if(fRet) {
            fRet = usbDriver.RegisterClientSettings(szDriverLibFile, TEXT("Generic UFN Test Loopback Driver"), NULL, &DriverSettings);
            ASSERT(fRet);
        }

    }

    USBMSG(LOG_DETAIL, _T("End USBInstallDriver return value = %d "), fRet);
    ASSERT(fRet);
    return fRet;
}
    ...
    while(dwTimeElapsed < dwRunTime)
    {
        ...
        if(pClientDrv)
        {
            ...
            if(g_UsbParam.dwClients & SERIAL_CLIENT)
                szTransfer = new StressTransfer(Serial_Transfer,pClientDrv,dwActiveDevice,rand(),1024);
            else if(g_UsbParam.dwClients & RNDIS_CLIENT)
                szTransfer = new StressTransfer(RNDIS_Transfer,pClientDrv,dwActiveDevice,rand(),1024);
            else if(g_UsbParam.dwClients & STORAGE_CLIENT)
                szTransfer = new StressTransfer(Storage_Transfer,pClientDrv,dwActiveDevice,rand(),1024);
            else
                szTransfer = new StressTransfer(Overload_Transfer,pClientDrv,dwActiveDevice,rand(),1024);
            szTransfer->ThreadStart();

            if(g_UsbParam.fClicker && (uint)rand() < (g_UsbParam.dwConnectDisconnectProbability * RAND_MAX) / 100)
            {
            ...
            while(dwStatus = WaitForSingleObject(szTransfer->hEvent,1000))
            {
...
extern "C" SHELLPROCAPI ShellProc(UINT uMsg, SPPARAM spParam)
{
    ...
    switch (uMsg) {
        ...
        case SPM_SHELL_INFO:
            ...
            g_szDllCmdLine = (LPSTR)_tcsdup(g_pShellInfo->szDllCmdLine);

            if(!ParseCommandLine(&g_UsbParam, (TCHAR *) g_szDllCmdLine))
            {
                 return TPR_FAIL;
                  }
...
BOOL ParseCommandLine(LPUSB_PARAM lpUsbParam, LPTSTR lpCommandLine)
{
    ...
    token = _tcstok(lpCommandLine, seps);
    while(token != NULL) {
        if(token[0] == TEXT('-')) {
            ...
            if(pswitch)
            {
                switch (*pswitch)
                {
                    ...
                    case _T('L'):
                        ...
                        _stscanf(token, TEXT("%i"), &lpUsbParam->dwConnectDisconnectProbability);
                    ...
                    case _T('T'):
                        if(_tcslen(token) > 2)
                        {
                            _stscanf(token, TEXT("-t%i"), &lpUsbParam->dwDuration);
                            RETAILMSG(1, (_T("Time to Run %d "), lpUsbParam->dwDuration));
                        }
                        else
                        {
                            token = _tcstok(NULL, seps);
                            _stscanf(token, TEXT("%i"), &lpUsbParam->dwDuration);
                            RETAILMSG(1, (_T("Time to Run %d "), lpUsbParam->dwDuration));
                        }
