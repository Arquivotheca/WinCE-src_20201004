...
BOOL GetCommandLine(TCHAR *szPlatName, FILE *fCmdFile, TCHAR *szCmdLine,
                                                       DWORD dwCmdLineLen);
...
BOOL handleCmdLine ()
{
  ...
  _tcsncat_s (szRealName, szFname, MAX_FILENAME);
  
  szRealName[MAX_FILENAME - 1] = 0;

  // szRealName should now have complete path to file
  // Open the file in binary read mode		 
  fCmdFile = _tfopen (szRealName, _T("rb"));
                                                        ...
                                                        DWORD dwCmdLineLen)
{
  ...
  TCHAR tc_szCmd[MAX_COMMAND_LENGTH];
  TCHAR szStoreCmdLine[MAX_COMMAND_LENGTH];
  
  do
  {
    if (!fgets(szCmd, countof (szCmd), fCmdFile))
    {
    ...
    *endPtr = '\0';
    ...
    mbstowcs_s(&unicodeLength, NULL, 0, szCmd, countof(szCmd));
    ...
    if(unicodeLength > countof(tc_szCmd))
    {
    ...
    unicodeLength = 0;
    if(mbstowcs_s (&unicodeLength, tc_szCmd, countof(tc_szCmd), szCmd, _TRUNCATE)
                                                 || (unicodeLength <= 0))
    {
    ...
    _tcsncpy_s(szStoreCmdLine, countof(szStoreCmdLine), tc_szCmd, _TRUNCATE);
  ...
  if(countof(szStoreCmdLine) > dwCmdLineLen)
  {
