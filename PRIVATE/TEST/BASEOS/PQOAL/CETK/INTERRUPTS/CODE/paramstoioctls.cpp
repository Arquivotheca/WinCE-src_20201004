...
sLargeCall LargeCallDynamic =
  {SYSINTR_UNDEFINED, OAL_INTR_DYNAMIC};
sLargeCall LargeCallStatic =
  {SYSINTR_UNDEFINED, OAL_INTR_STATIC};
sLargeCall LargeCallForceStatic =
  {SYSINTR_UNDEFINED, OAL_INTR_FORCE_STATIC};
...
runBadInputNewModel_IncompleteArgSets()
{
  ...
  for (DWORD dwRow = 0; dwRow < countof (newModelIncompleteArgSets); dwRow++)
    {
...
runBadInputNewModel_WrongInSizes ()
{
  ...
  if (!runBadInputNewModel_WrongInSizes (OAL_INTR_FORCE_STATIC, 
  ...
  if (!runBadInputNewModel_WrongInSizes (OAL_INTR_DYNAMIC, 
					  _T("OAL_INTR_DYNAMIC")))
    {
				  ...
				  const TCHAR *const szFlagName)
{
  ...
  DWORD dwInBuf [MAX_INBOUND_BUF_LENGTH_DW];  
  DWORD dwOutBuf;
  ...
  /* used many times below */
  PREFAST_SUPPRESS (326, "by design");
  if (MAX_INBOUND_BUF_LENGTH_DW < 2)
    {
      IntErr (_T("MAX_INBOUND_BUF_LENGTH_DW must be > 2"));
      return (FALSE);
    }
...
runBadFlags ()
{
  ...
  DWORD dwInBuf [MAX_INBOUND_BUF_LENGTH_DW];  
  DWORD dwOutBuf;		// this is just used for something to
	    ...
	    BOOL bTestLpBytesReturned)
{
  ...
  DWORD dwOutBuf [MAX_INBOUND_BUF_LENGTH_DW];
  ...
  if (bExpectToPass)
    {
      ...
      for (DWORD dwSize = sizeof (DWORD); 
	   dwSize < MAX_INBOUND_BUF_LENGTH_DW * sizeof (DWORD); dwSize++)
	{
			  ...
			  DWORD dwNumOfDwords)
{
  ...
  if (dwNumOfDwords == 0) return (TRUE);

  pdwBuf[0] = SYSINTR_UNDEFINED;

  if (dwNumOfDwords == 1) return (TRUE);

  /* need to assume something.  use dynamic.  arbitrary. */
  pdwBuf[1] = OAL_INTR_DYNAMIC;

  return (fillInBoundArrayOldModel (pdwBuf + 2, dwNumOfDwords - 2));
			  ...
			  DWORD dwNumOfDwords)
{
  ...
  for (BOOL bJustRolled = FALSE;
       dwIRQ <= dwIRQEnd && !bJustRolled; dwIRQ++)
    {
      ...
      DWORD dwInBuf[3] = {-1, OAL_INTR_STATIC};
      dwInBuf[2] = dwIRQ;
			    ...
			    NULL, NULL, NULL))
	{
      ...
      PREFAST_SUPPRESS (2015, "See check right below");
      pdwBuf[dwArrayPosToFill] = dwIRQ;

      if (dwArrayPosToFill == dwNumOfDwords - 1)
	{
	  /* done filling when position equals length - 1 */
	  break;
	}
...
runBadInSize_IOCTL_HAL_RELEASE_SYSINTR ()
{
  ...
  if (inKernelMode())
      ...
      if (!fillInBoundArrayOldModel (&dwGoodIRQ, 1))
	{
      ...
      DWORD dwBuf[3] = {-1, OAL_INTR_STATIC};
      dwBuf[2] = dwGoodIRQ;
      if (!KernelIoControl (IOCTL_HAL_REQUEST_SYSINTR, 
			    dwBuf, sizeof (dwBuf), 
			    &dwSysIntr, sizeof (dwSysIntr), NULL))
...
runBadInSize_IOCTL_HAL_REQUEST_IRQ ()
{
  ...
  DWORD dwInBuf [sizeof (DEVICE_LOCATION) * 4];  
...
runBadOutSize_IOCTL_HAL_REQUEST_IRQ ()
{
  ...
  DWORD dwInBuf [sizeof (DEVICE_LOCATION)];
