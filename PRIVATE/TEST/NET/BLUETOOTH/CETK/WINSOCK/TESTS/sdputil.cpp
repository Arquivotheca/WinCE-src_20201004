	...
	IN			NodeData			*pNodeData)
{
	...
	case SDP_ATTRIB_DOCUMENTATION_URL:
		...
		{
			safe_mbstowcs(pSDPUtilRecord->DocumentationURL, pNodeData->u.url.val, pNodeData->u.url.length);
		}
		else
		{
			OutStr(TEXT("Type mismatch for attribute 0x%04x. Expected 0x%04x 0x%04x. Got 0x%04x 0x%04x.\n"), 
	...
	case SDP_ATTRIB_CLIENT_EXECUTABLE_URL:
		if ((pNodeData->type == SDP_TYPE_URL) && (pNodeData->specificType == SDP_ST_NONE))
		{
			safe_mbstowcs(pSDPUtilRecord->ClientExecutableURL, pNodeData->u.url.val, pNodeData->u.url.length);
		}
		else
		{
			OutStr(TEXT("Type mismatch for attribute 0x%04x. Expected 0x%04x 0x%04x. Got 0x%04x 0x%04x.\n"), 
	...
	case SDP_ATTRIB_ICON_URL:
		if ((pNodeData->type == SDP_TYPE_URL) && (pNodeData->specificType == SDP_ST_NONE))
		{
			safe_mbstowcs(pSDPUtilRecord->IconURL, pNodeData->u.url.val, pNodeData->u.url.length);
		}
		else
		{
			OutStr(TEXT("Type mismatch for attribute 0x%04x. Expected 0x%04x 0x%04x. Got 0x%04x 0x%04x.\n"), 
	...
	case 0x0100:
		...
		{
			safe_mbstowcs(pSDPUtilRecord->ServiceName, pNodeData->u.str.val, pNodeData->u.str.length);
		}
		else
		{
			OutStr(TEXT("Type mismatch for attribute 0x%04x. Expected 0x%04x 0x%04x. Got 0x%04x 0x%04x.\n"), 
	...
	case 0x0101:
		if ((pNodeData->type == SDP_TYPE_STRING) && (pNodeData->specificType == SDP_ST_NONE))
		{
			safe_mbstowcs(pSDPUtilRecord->ServiceDescription, pNodeData->u.str.val, pNodeData->u.str.length);
		}
		else
		{
			OutStr(TEXT("Type mismatch for attribute 0x%04x. Expected 0x%04x 0x%04x. Got 0x%04x 0x%04x.\n"), 
	...
	case 0x0102:
		if ((pNodeData->type == SDP_TYPE_STRING) && (pNodeData->specificType == SDP_ST_NONE))
		{
			safe_mbstowcs(pSDPUtilRecord->ProviderName, pNodeData->u.str.val, pNodeData->u.str.length);
		}
		else
		{
			OutStr(TEXT("Type mismatch for attribute 0x%04x. Expected 0x%04x 0x%04x. Got 0x%04x 0x%04x.\n"), 
