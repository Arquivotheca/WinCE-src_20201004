...
HRESULT CDispatchHolder::Init(void)
{
    ...
    if (m_bCachable)
    {
        ...
        stringChk = StringCchCopyW(achBuffer, _MAX_BUFFER_SIZE, _T("CLSID\\")); 
        ASSERT(SUCCEEDED(stringChk));
        stringChk = StringCchCatW(achBuffer, _MAX_BUFFER_SIZE, p); 
        ASSERT(SUCCEEDED(stringChk));
        stringChk = StringCchCatW(achBuffer, _MAX_BUFFER_SIZE, _T("\\InProcServer32")); 
        ASSERT(SUCCEEDED(stringChk));
        ...
        if (RegOpenKeyEx(HKEY_CLASSES_ROOT, achBuffer, 0, KEY_READ, &hKey.hkey)==ERROR_SUCCESS)
        {
            if (RegQueryValueEx(
                ...
                )==ERROR_SUCCESS)
                {
                 ...
                 if (dwSize>0 && (_wcsicmp(achBuffer, _T("Both"))==0 
                             || _wcsicmp(achBuffer, _T("Free"))==0 
                             || _wcsicmp(achBuffer, _T("Neutral"))==0))
                             {
                                // correct   
                                   m_bCachable = true;
                             }
