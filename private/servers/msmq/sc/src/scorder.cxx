...
void ScSequenceCollection::SendAcks (void) {
    ...
    while (pSeq) {
        SVSUTIL_ASSERT (pSeq->fActive);
        SVSUTIL_ASSERT_PREFAST (pSeq->p);
        SVSUTIL_ASSERT_PREFAST (pSeq->pQueue);
...
int ScSequenceCollection::AddNewPairToHash (WCHAR *szString, GUID *pguid, int fAddToFile) {
    ...
    unsigned int cchString = 1 + wcslen(szString);
    StringGuidHash *pH = (StringGuidHash *)g_funcAlloc (offsetof (StringGuidHash, szString) + sizeof(WCHAR)*cchString, g_pvAllocData);
    if (! pH) {
#if defined (SC_VERBOSE)
    ...
    pH->guid = *pguid;
    pH->uiRef = 0;
    HRESULT hr = StringCchCopyW (pH->szString, cchString, szString);
    ASSERT(SUCCEEDED(hr));
...
void ScSequenceCollection::PruneStringHash (void) {
    ...
    if (iEmpty * SCSEQUENCE_STALEFACTOR > iTotal) {        // Rewrite
        ...
        HRESULT hr = StringCchCopyW(szNewFile, _countof(szNewFile), lpszStringName);
        ASSERT(SUCCEEDED(hr));
        hr = StringCchCatW (szNewFile, _countof(szNewFile), L".new");
        ASSERT(SUCCEEDED(hr));
