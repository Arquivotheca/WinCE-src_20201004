...
//Default timeout in milliseconds in VerifyPowerState function. 
//This value can be set in VerifyPowerState's parameter.
#define TIMEOUT_VERIFYPOWERSTATE 3000

//Polling interval in milliseconds in VerifyPowerState function.
#define POLLTIME_VERIFYPOWERSTATE 1000

////////////////////////////////////////////////////////////////////////////////
/********************  Helper Functions  ************************/

BOOL VerifyPowerState(LPCWSTR pszPwrStateName, DWORD dwTimeout = TIMEOUT_VERIFYPOWERSTATE);
...
BOOL VerifyPowerState(LPCWSTR pszPwrStateName, DWORD dwTimeout)
{
    ...
    // Now Get Power State to check if the Power State was indeed set
    if (ERROR_SUCCESS != GetSystemPowerState(szStartState, _countof(szStartState), &dwGetFlags))
    {
    ...
    while (1)
    {
        ...
        {
            PWRMSG(LOG_DETAIL, _T("Failed to transition to state \"%s\" after waiting %u ms"), pszPwrStateName, dwElapsed);
            return FALSE;
        }
        if (ERROR_SUCCESS != GetSystemPowerState(szCurState, _countof(szCurState), &dwGetFlags))
        {
            PWRMSG(LOG_DETAIL, _T("GetSystemPowerState() returned FALSE. GetLastError = %d"), GetLastError());
            return FALSE;
        }
        // Verify that it is the expected power state
        if (0 == _tcsnicmp(szCurState, pszPwrStateName, _tcslen(szCurState) + 1))
        {
        ...
        {
            //Power state should not transition to other state than expected state
            if (0 != _tcsnicmp(szCurState, szStartState, _tcslen(szCurState) + 1))
            {
                PWRMSG(LOG_DETAIL, _T("Incorrectly transitioned to state \"%s\" from state \"%s\""), szCurState, szStartState);
                return FALSE;
            }
            Sleep(POLLTIME_VERIFYPOWERSTATE);
        }
    }
}
