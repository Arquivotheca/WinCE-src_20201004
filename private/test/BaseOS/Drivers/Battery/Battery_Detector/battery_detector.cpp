...
//
//
// Battery technology detector for TestKit
//
//
...
#define __BIN_NAME__ TEXT( "Battery_Detector.dll")
...
extern "C"{
    FUNCTION_TABLE_ENTRY g_lpFTE[] = {
        TEXT( "Battery Driver Technology Detector" ), 0, 0, BVT_BASE+0, DetectorTest,
        {NULL, 0, 0, 0, NULL}
    };
}
...
BOOL IsTechPresent(void)
{
    ...
    GUID guidBattery = { 0xdd176277, 0xcd34, 0x4980, { 0x91, 0xee, 0x67, 0xdb, 0xef, 0x3d, 0x89, 0x13 } };
    DEVMGR_DEVICE_INFORMATION ddiBattery;

    memset(&ddiBattery, 0, sizeof(DEVMGR_DEVICE_INFORMATION));
    ddiBattery.dwSize = sizeof(DEVMGR_DEVICE_INFORMATION);

    HANDLE hDevice = FindFirstDevice(DeviceSearchByGuid, &guidBattery, &ddiBattery);
    ...
    if (bFlag == TRUE)
        DETAIL("Successfully detected Battery Driver.");
    else
        WARN("Did not detect Battery Driver.");
