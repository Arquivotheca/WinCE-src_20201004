...
SHELLPROCAPI ShellProc( UINT uMsg, SPPARAM spParam ) 
{
    ...
    {    
        ...
        case SPM_LOAD_DLL: 
			...
			//Check if the accelerometer driver exists
			g_bAccDriverExist = CheckAccDriverExist();
			if(!g_bAccDriverExist)
			{
				LOG( "No Accelerometer Driver Found: Skipping All Tests" );
			}
        ...
        case SPM_BEGIN_TEST:
			...
			if(!g_bAccDriverExist)
			{
				LOG( "No Accelerometer Driver Found: Skipping Test" );
				iReturn = SPR_SKIP;
			}
				
            if( (pBT->lpFTE->dwUniqueID != 3014) && g_bAccDriverExist )
            {
        ...
        case SPM_END_TEST:
            ...
            if( (pET->lpFTE->dwUniqueID != 3014) && g_bAccDriverExist )
            {
                AccGetSystemPowerState();
