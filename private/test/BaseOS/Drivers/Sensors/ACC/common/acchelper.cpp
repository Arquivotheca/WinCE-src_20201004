...
// CheckAccDriverExist()
//
// Function to check if the accelerometer driver exists

BOOL CheckAccDriverExist()
{
	BOOL result;
	LUID sensorLuid = {0};
    HSENSOR hAcc = ACC_INVALID_HANDLE_VALUE;
	
	result = AccGetHandle( hAcc, sensorLuid );	//Try to get a handle
	CHECK_CLOSE_ACC_HANDLE(hAcc);				//Close the handle
	return result;
}
...
BOOL ProcessCmdLine(LPCWSTR pszCmdLine)
{
    ...
    g_AccParam.bFakePdd = IsModuleInstalled( _T("accdrvr.dll"));    
