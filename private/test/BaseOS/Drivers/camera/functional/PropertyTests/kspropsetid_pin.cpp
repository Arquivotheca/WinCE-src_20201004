...
TESTPROCAPI Test_PIN_DATARANGES( UINT uMsg, TPPARAM tpParam, LPFUNCTION_TABLE_ENTRY lpFTE )
{
    ...
    // uCount is passed into GetDataRanges, which takes the test's pin identifier, not the real pin id.
    // so, test all three pins and if there's no available pin instance for one of the ones being tested, skip it.
    for ( USHORT uCount = 0 ; uCount < 3 ; uCount++ )
    {
    //    For each pin, determine the dataranges
        Log( TEXT( "Test_PIN_DATARANGES : Testing Pin %u" ), uCount ) ;

        if(!camTest.AvailablePinInstance(uCount))
        {
            Log(TEXT("Pin %u not available, continuing test."), uCount);
            continue;
        }
        ...
        if ( FALSE == camTest.GetDataRanges( uCount /*PIN ID*/,
        ...
        do
        {
            Log( TEXT( "Test_PIN_DATARANGES : Testing Pin %u, DataRange %d" ), uCount, iIndex ) ;
        ...
        } while ( iIndex != (INT)pCSMultipleItem->Count ) ;
    }
