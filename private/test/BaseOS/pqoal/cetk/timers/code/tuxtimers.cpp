...
#define DRIFT_TEST_RUN_TIME_S (60 * 60 * 1)
...
runResolution(sTimer * timer,
          DWORD dwIterations,
          ULONGLONG * pullN, ULONGLONG * pullRange, DWORD *pdwRangeCount,
          int iNewPriority);
...

{
    ...
    if (!runResolution(&timer, GTC_RES_RUN_INTERATIONS, &ullN, &ullRange, &dwRangeCount, 1))
    {
        Error (_T("Couldn't get resolution for timer %s"), timer.name);
        goto cleanAndExit;
    }

    if ((ullN > ullMaxN_ms) || (ullRange > ullMaxRange) || (dwRangeCount > GTC_RES_RUN_INTERATIONS * 5/100))
    {
        Error (_T("The resolution of %s is out of bounds."), timer.name);
        Error (_T("It should be between [%I64u, %I64u] ticks but is instead ")
         _T("[%I64u, %I64u] which occur %d times"),
         ullMaxN_ms, ullMaxN_ms + ullMaxRange, ullN, ullN + ullRange, dwRangeCount);
          ...
          int iNewPriority)
{
    ...
    BOOL rVal = getResolution(timer, dwIterations, pullN, pullRange, pdwRangeCount, TRUE);
