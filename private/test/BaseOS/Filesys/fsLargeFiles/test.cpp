...
BOOL Test_CopyLargeFile()
{
    ...
    DWORD           dwFileSize;
    DWORD           dwFileSizeHigh;
	ULARGE_INTEGER ulfreeSpace ={0}, ulFilesize = {0};
	ULARGE_INTEGER ultotalBytes = {0}, ultotalFreeBytes = {0};
	...
	//Open file that is being copied and check the file size
	hFile = CreateFile(szBuf1, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                       OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);

    if (hFile == INVALID_HANDLE_VALUE)
    {
        LOG(TEXT("WARNING!!!::Couldn't open file %s. GLE = %d"), szBuf1, GetLastError());
		LOG(TEXT("Please be sure to run test case 1001 first"));
        return FALSE;
    }
	dwFileSize = GetFileSize(hFile, &dwFileSizeHigh);
	ulFilesize.HighPart = dwFileSizeHigh;
	ulFilesize.LowPart = dwFileSize;
	//Done with the file. Close handle
		CloseHandle(hFile);

	//Get the free space on the store and print it to the log
	if(!GetDiskFreeSpaceEx(g_szHardDiskRoot, &ulfreeSpace, &ultotalBytes, &ultotalFreeBytes))
	{
       LOG(TEXT("WARNING!!!::Couldn't get the disk space. GLE = %d"), GetLastError());
    }
	else
	{
		LOG(TEXT("The total Free disk space is = %I64u Bytes"), ultotalFreeBytes.QuadPart);
	}
	LOG(TEXT("The file size being copied is = %I64u Bytes"), ulFilesize.QuadPart);
