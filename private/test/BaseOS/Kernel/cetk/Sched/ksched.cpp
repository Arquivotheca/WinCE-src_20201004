    ...
    )
{
    ...
    for (i = 0; i < gdwNumThreads; i++) {
        ...
        pThreadInfo[i].dwThreadPrio = (i == gdwNumThreads - 1) ? (gdwHighestPrio + 1) : rand_range(gdwHighestPrio + 2, gdwLowestPrio);
        if(!CeSetThreadPriority (pThreadInfo[i].hThread, pThreadInfo[i].dwThreadPrio)) {
    ...
    WaitForSingleObject(pThreadInfo[gdwNumThreads-1].hThread, INFINITE);
