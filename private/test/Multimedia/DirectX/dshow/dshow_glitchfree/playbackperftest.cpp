...
TESTPROCAPI CPlaybackTest::Execute(PerfTestConfig* pConfig)
{
    ...
    for(int rep = 0; rep < pConfig->GetRepeat(); rep++)
    {
        ...
        for(int clipi = 0; clipi < nClipIDs; clipi++)
        {
            ...
            {
                if (bAllProtocols) {
                    errno_t err = _tcsncpy_s(szProto, MAX_PATH, SupportedProtocols[protoi], _TRUNCATE);
                    if(err == 0 || err == 80)
                    {
                                                    ...
                                                    PlaybackStats *pStats)
{
    ...
    if(SUCCEEDED(hr)) 
    {
        ...
        {
            if((abs((int)(pMediaContent->m_QualityControl.m_dDuration - pStats->actualDuration)) > pMediaContent->m_QualityControl.m_dDurationMarginOfError) && 
