...
{          
    ...
    TCHAR *token = _tcstok_s(szMediaPath, TEXT("\\"), &next_token);
    int nIndex = 0;
   ...
   while(nIndex>0)
   {
       ...
       _tcsncat_s(RootPath, MAX_PATH, TEXT("\\"), countof(TEXT("\\")));
       for (int nIndex2=0; nIndex2<nIndex; nIndex2++)
       {      
            _tcsncat_s(RootPath, MAX_PATH, szNodeList[nIndex2], _tcslen(szNodeList[nIndex2]));
            if(nIndex2 != (nIndex-1))
            _tcsncat_s(RootPath, MAX_PATH, TEXT("\\"), countof(TEXT("\\")));
       }
...
DWORD AnalyzeSystemMonitoringData(DWORD MemoryLoadRecord[], DWORD PhyMemoryUseRecord[], DWORD VirMemoryUseRecord[], int CPUUseRecord[], UINT RecordCount)
{
    ...
    {
        LOG ( TEXT("Not a valid SysmonInfo record to analyze the memory and cpu usage. Integer underflow, RecordCount-3 = %u is greater than RecordCount = %u"), RecordCount-3, RecordCount);
    }
    ...
    LOG ( TEXT("where value withing [] is the index of the data point"));
            ...
            DWORD AvgPhyMemUseTail = 0;
            if(RecordCount >= 5)
            {
                AvgPhyMemUseTail = (PhyMemoryUseRecord[RecordCount-3] + 
                PhyMemoryUseRecord[RecordCount-4] + 
                PhyMemoryUseRecord[RecordCount-5])/3;
            }
            else
            {
                LOG ( TEXT("Not a valid SysmonInfo record to analyze the memory and cpu usage. Integer underflow, RecordCount-5 = %u is greater than RecordCount = %u"), RecordCount-5, RecordCount);
            }
            ...
            if(MaxPhyMemIndex > 2)
            {
                if(RecordCount >= 3)
                {
                    if( MaxPhyMemIndex < (RecordCount - 3) )
                    {
