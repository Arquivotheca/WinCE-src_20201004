...
OutputLogInfo(cCompareResults * cCr)
{
    ...
    for(int i = 0; NULL != (tszLoginfo = cCr->GetLogLine(i)); i++)
    ...
    if (g_fUsePrimaryDisplay)
    {
        ...
        if (gRotateAvail)
        {
            if(glCurrentAngle == -1)
                ...
                info(DETAIL, TEXT("Display rotation available, current angle is %d."), glCurrentAngle);
        }
        else
...
IncreaseProgramMemory()
{
    ...
    dwTmp = SetSystemMemoryDivision((DWORD) ceil((double)(dwStorageToSet/dwPageSize)));
