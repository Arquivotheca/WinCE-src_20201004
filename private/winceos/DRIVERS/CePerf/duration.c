    ...
    if (pDL->dwNumVals > 0) {
        ...
        if ((pItem->dwRecordingFlags & CEPERF_RECORD_ABSOLUTE_CPUPERFCTR)
            ...
            for (wCounter = 0;
                ...
                // Print value names -- won't overflow the buffer
                swprintf_s(szSum, 12,    TEXT("CPU%02uSum"),    wCounter);
                swprintf_s(szMin, 12,   TEXT("CPU%02uMin"),    wCounter);
                swprintf_s(szMax, 12,   TEXT("CPU%02uMax"),    wCounter);
                swprintf_s(szStdDev, 15, TEXT("CPU%02uStdDev"), wCounter);
