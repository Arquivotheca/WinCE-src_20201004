...
// Operator+= -- Add to another EPOINTFIX.

    VOID operator+=(const EPOINTFIX& eptfx)
    {
        x += eptfx.x;
        y += eptfx.y;
    }
...
// operator== -- checks that the two coordinates are equal
    ...
    BOOL operator==(const POINTL& ptl)
    {
        return(x == ptl.x && y == ptl.y);
    }

// operator- -- Assing a POINTL to this instance.

    EPOINTL& operator=(const POINTL& ptl)
    {
        x = ptl.x;
        y = ptl.y;
        return *this;
