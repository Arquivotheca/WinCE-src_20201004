...
KillShadow()
{
    ...
    if (didit)
    {
        ...
        if (notify)
        {
...
#if !defined(_M_ARM)
            if (!user32 && GetLastError() == ERROR_INVALID_PARAMETER)
            {
            ...
            if (!user32)
                return;
            typedef int (*pMsgBoxProc)(HWND,LPCWSTR,LPCWSTR,UINT);
            pMsgBoxProc pMsgBox = (pMsgBoxProc)GetProcAddress(user32, "MessageBoxW");
            if (!pMsgBox)
                return;
            pMsgBox(NULL, L"The Advanced Memory Checking subsystem has run out of virtual memory,"
                          L" and is now disabled. The checks will no longer occur for this process. "
                          L"Try freeing up hard drive space for your swap file.",
                    L"RTC Subsystem Failure",
