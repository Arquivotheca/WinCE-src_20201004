...
*       10-01-04  AGH   New prototype for _RTC_AllocaFailure
...
#define INIT_LOCK   InitializeCriticalSectionAndSpinCount(&_RTC_memlock, _CRT_SPINCOUNT)
...
_RTC_error_fnW      _RTC_GetErrorFuncW(LPCVOID addr);
void                _RTC_StackFailure(void *retaddr, const char *varname);
void                _RTC_AllocaFailure(void *retaddr, _RTC_ALLOCA_NODE * pn, int num);
BOOL                _RTC_GetSrcLine(LPBYTE address, _Out_writes_(sourcelen) wchar_t* source, DWORD sourcelen,
                                    int* pline, _Out_writes_(modulelen) wchar_t* moduleName, DWORD modulelen);
extern "C" {
...
#ifdef _WIN32_WCE //it has defined in winerror.h also
...
#define HelloVC( exinfo )   \
    RaiseException( EXCEPTION_VISUALCPP_DEBUGGER, 0, sizeof(exinfo)/sizeof(DWORD), (ULONG_PTR *)&exinfo )
...
/* must use pack 8 on 64 bits platforms */
#if defined(_WIN64)
#pragma pack(push, 8)
#endif  /* _WIN64 */
