    ...
    )
{
    ...
    if (LoadString (pMod->hModule, 1, szBuffer, MAX_PATH))
    {
        return (0 == _tcscmp (_T("AppVerifier"), szBuffer));
             ...
             _tcsstr (g_pszProgName, _T("cerldcli.exe")))
    {
        return TRUE;
    }
    ...
    )
{
    ...
    if (!m_hCRT) {
        // CRT haven't been loaded yet. This is during initialization time and coredll/msvcrt might not be
        // ready yet. We can't load shim in this stage as the shim DLLs can use both coredll/msvcrt functions.
        return;
    }
