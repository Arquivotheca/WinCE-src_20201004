...
BOOL g_fPgfEnabled; // is page file support enabled in this process
...
extern "C"
BOOL
xxx_SetForegroundProcess (
    HPROCESS      hProcess
    )
{
    BC_PROC (hProcess);
    return SetMemoryPriority (hProcess, MEMORY_PRIORITY_FOREGROUND);
}

extern "C"
BOOL
xxx_SetSystemProcess (
    HPROCESS      hProcess
    )
{
    BC_PROC (hProcess);
    return SetMemoryPriority (hProcess, MEMORY_PRIORITY_SYSTEM);
}
...
extern "C"
BOOL
xxx_RegisterOOMNotification (PHANDLE phMemLow, PHANDLE phMemPressure, PHANDLE phGoodMem)
{
    return RegisterOOMNotification (phMemLow, phMemPressure, phGoodMem);
}

extern "C"
BOOL
xxx_IsSystemProcess (DWORD dwProcId)
{
    return IsSystemProcess (dwProcId);
}
...
extern "C"
BOOL WINAPI CeGetProcessMemoryInfo (HPROCESS hProcess, PCE_PROCESS_MEMORY_COUNTERS pCounters)
{
    BC_PROC (hProcess);
    return NKGetProcessMemoryInfo (hProcess, pCounters, pCounters->cb);
}
...
static BOOL DoTerminateProcess(HPROCESS hProc, DWORD dwRetVal, BOOL fWait)
{
#ifndef KCOREDLL
...
#endif
    BC_PROC(hProc);
    BOOL fRet = TerminateProcess (hProc, dwRetVal);

    if (fRet && fWait && (WAIT_OBJECT_0 != xxx_WaitForSingleObject (hProc, 3000))) {
...
extern "C"
BOOL WINAPI xxx_TerminateProcess(HPROCESS hProc, DWORD dwRetVal)
{
    return DoTerminateProcess (hProc, dwRetVal, TRUE);
}

extern "C"
BOOL WINAPI xxx_TerminateProcessNoWait (HPROCESS hProc, DWORD dwRetVal)
{
    return DoTerminateProcess (hProc, dwRetVal, FALSE);
}
    ...
    )
{
    ...
    BOOL fCurProcess = (((HANDLE)GetCurrentProcessId() == hModule) || (GetCurrentProcess() == hModule));
    ...
    BC_PROC (hProc);
    fRet = PageOutModule (hProc, hModule, dwFlags);

    // mark pgf flag for this process (used in deciding heap compaction)
    g_fPgfEnabled |= (fRet && fCurProcess && (dwFlags & PAGE_OUT_PROCESS_DATA));
...
{
    BC_PROC (hProc);
    if (MINFO_PEHEADER == infoType) {
        return ReadPEHeader (hProc, 0, (DWORD)lpBaseAddr, pBuf, cbBufSize);
    } else {
