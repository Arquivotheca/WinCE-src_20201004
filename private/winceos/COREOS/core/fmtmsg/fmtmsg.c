...
//TODO: VS11 porting: This redefinition needs to be in some common header file for coredll
#ifdef _WCE_CORECRT
#undef _swprintf
extern int (*mpfn_CRT_swprintf)(wchar_t *, const wchar_t *, ...);
#define _swprintf (*mpfn_CRT_swprintf)
#endif
...
BOOL vmprintf(vmbuf *pDst, LPCWSTR pStr, LPCVOID p1, LPCVOID p2, LPCVOID p3)
{
    ...
    try {
        // the use of _swprintf here is safe and necessary, since this call is try/excepted and relies on faults
...
#pragma warning(disable:4996) // C4996: Unsafe API warning
        len = _swprintf((LPWSTR)pDst->pCur,pStr,p1,p2,p3);
        /* _swprintf return -1 indicate error case or WEOF 0xffff */
        if (len >= 0) {
            fRet = TRUE;
