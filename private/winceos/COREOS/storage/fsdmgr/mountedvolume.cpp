...
LRESULT MountedVolume_t::FinalizeMount ()
{
    ...
    // load error handler
    m_pDisk->LoadErrorHandler (MountName, m_MountFlags);
...
{
    DEBUGMSG (ZONE_INIT, (L"FSDMGR!MountedVolume_t::CloseAllHandles: Close all handles for volume(%p)\r\n", this));
    LockVolume ();

    for (FileSystemHandle_t* pHandle = m_HandleList.FirstItem (); pHandle != NULL;) {
        ...
        if (FileSystemHandle_t::HDL_FILE & HandleFlags) {
            DEBUGMSG (ZONE_INIT, (L"FSDMGR!MountedVolume_t::CloseAllHandles: Close a file handle 0x%x\r\n", HandleContext));
            // Close a file handle.
            ...
            DEBUGMSG (ZONE_INIT, (L"FSDMGR!MountedVolume_t::CloseAllHandles: Close a search handle 0x%x\r\n", HandleContext));
            pFileSystem->FindClose (HandleContext);

        } // else this must be a pseudo handle.

        DEBUGMSG (ZONE_INIT, (L"FSDMGR!MountedVolume_t::CloseAllHandles: before pHandle->Invalidate()\r\n", pHandle));
        // Mark the handle as invalid.
        pHandle->Invalidate ();

        // first move pHandle since once signaled it may be deleted
        FileSystemHandle_t* pHandleCopy = pHandle;
        pHandle = m_HandleList.NextItem (pHandle);
        pHandleCopy->SignalVolumeClosedThisHandleEvent();
    }
...
void MountedVolume_t::RemoveHandle (FileSystemHandle_t* pHandle)
{
    ...
    DEBUGMSG (ZONE_INIT, (L"FSDMGR!MountedVolume_t::RemoveHandle: m_HandleList.RemoveItem (0x%x), cnt=%d", pHandle, m_HandleList.GetItemCount()));
