...
{       
    ...
    CopyOutParam<WIN32_FIND_DATAW> FindData(pFindData);
    ...
    if (ERROR_SUCCESS == lResult) {
        ...
        //
        HANDLE hInt = pFileSystem->FindFirstFileW (hProcess, pPathName, &FindData);
        ...
        switch (Fsctl) {
            ...
            default:
                fRet = pFileSystem->FsIoControl (hProc, Fsctl, pInBuf, InBufSize, pOutBuf,
                    OutBufSize, pBytesReturned, &Overlapped);
                break;
        }
