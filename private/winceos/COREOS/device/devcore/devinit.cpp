...
//Do a quick check to see if we're in BootPhase2
...
DWORD DeviceContent::LoadLib(LPCTSTR RegPath)
{
    ...
    if ((dwFlags & DEVFLAGS_LOAD_AS_USERPROC)) {
        //If we're using two BootPhases, there can be a race condition (after "SYSTEM/BootPhase2" is signaled, but g_BootPhase has not been updated)
        //If g_BootPhase is <2, we'll have to confirm it.
        //We can fix this race by introducing yet another event in DM, but we will just acknowledge the benign race and handle it.
        if (g_BootPhase<2 && !IsInBootPhase2()) {
            ASSERT(FALSE);
...
)
{
    ...
    if (!m_DeviceNameString) {
        ...
        if((dwFlags & DEVFLAGS_NAKEDENTRIES) == 0) {
            if(lpszPrefix[0] != 0) {
                DEBUGCHK(lpszBusPrefix[0] == 0 || _wcsicmp(lpszBusPrefix, lpszPrefix) == 0);
