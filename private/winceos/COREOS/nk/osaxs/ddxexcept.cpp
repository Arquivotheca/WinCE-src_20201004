                            ...
                            )
{
    ...
    HRESULT hr = GetBucketParameters(&g_ceDmpDDxBucketParameters, pth, (pth ? pth->pprcActv : NULL));
                             ...
                             )
{
    ...
    ddxlog(L" Active Process (owns module) : %s\r\n",     (pth && pth->pprcActv) ? pth->pprcActv->lpszProcName : L"<unknown>");
    ddxlog(L" Owner Process (owns thread)  : %s\r\n",     (pth && pth->pprcOwner) ? pth->pprcOwner->lpszProcName : L"<unknown>");
