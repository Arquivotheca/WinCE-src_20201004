...
static HRESULT WriteProcessData (PROCESS *pProc, DWORD &riOut, const DWORD cbOut, BYTE *pbOut)
{
    ...
    if (pbOut)
    {
        ...
        if (pProc->lpszProcName)
        {
            ...
            {
                if(strcpy_s (proc.szName, _countof(proc.szName), "Unknown"))
                {
                    return E_FAIL;
                }
            }
        }
        else if(strcpy_s (proc.szName, _countof(proc.szName), "Unknown"))
        {
            return E_FAIL;
        }
        ...
        if (strcpy_s (proc.szCommandLine, _countof(proc.szCommandLine), "not available"))
        {
            return E_FAIL;
        }
...
static HRESULT WriteProcessDataAsModule (PROCESS *pProcess, DWORD &riOut, const DWORD cbOut, BYTE *pbOut)
{
    ...
    if (pbOut)
    {
        ...
        __except (EXCEPTION_EXECUTE_HANDLER)
        {
            if(strcpy_s (mod.szModuleName, _countof(mod.szModuleName), "Unknown"))
            {
                return E_FAIL;
            }
        }
    ...
    DWORD &riOut, const DWORD cbOut, BYTE *pbOut, DWORD *pcElements)
{
    ...
    pmodlist = g_pprcNK ? g_pprcNK->modList.pFwd : NULL;
