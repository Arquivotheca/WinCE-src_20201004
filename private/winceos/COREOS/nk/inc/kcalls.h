...
// used by srw lock code to suspend self when waiting to acquire a srw lock
BOOL SCHL_ThreadBlockSRWLock (void);

// resume a thread waiting on a srw lock or set a bit to resume thread on next block
...
// raise thread priority to be at least equal to nPriority
