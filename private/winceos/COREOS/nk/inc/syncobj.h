...
#define EXTERNAL_WAIT_ONLY      0x4     // bit field in manualreset, indicate this is a process event
...
#define OwnCS(lpcs)    (OWNER_ID_OF_CS (lpcs) == dwCurThId)
...
// Kernel should NEVER reference the OwnerThread field of a CS
#define OwnerThread                    +++ DO NOT USE +++
...
//
// (internal only) fast event set/reset. RACE CAN OCCUR, CALLER IS RESPONSIBLE FOR RACE HANDLING 
//
...
FORCEINLINE void FastResetEvent (PEVENT pEvt)
{
    if (pEvt->state) {
        ForceEventModify (pEvt, EVENT_RESET);
    }
}
