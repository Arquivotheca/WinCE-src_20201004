...
BOOL APISRegister (PAPISET pas, DWORD dwSetID)
{
    ...
    if (dwErr) {
        ...
        PPROCESS pprc = pActvProc;
        // signal the "new API registered" event and update the process status to "PSL server"
        SetProcessFlags (pprc, PROC_PSL_SERVER);
        if (pprc != g_pprcNK) {
...
void NKPSLNotify (DWORD dwFlags, DWORD dwProcId, DWORD dwThrdId)
{
    ...
    pth->tlsSecure[TLSSLOT_KERNEL] |= TLSKERN_IN_PSLNOTIFY;
    ...
    if (!pKrnStk || !pSvrStk) {
        ...
        if (!dwErr) {
            ...
            *(LPDWORD)dwKrnSP = SYSCALL_RETURN;
            ...
            pKrnTls[TLSSLOT_RUNTIME]   = (DWORD) RTGlobFromTls (pSvrTls);
...
void UnwindOneCstk (PCALLSTACK pcstk, BOOL fCleanOnly)
{
    ...
    if (CST_UMODE_SERVER & pcstk->dwPrcInfo) {
        ...
        if ((pprcServer != pcstk->pprcLast) || (pSvrTls != pcstk->pOldTls)) {
            ...
            VERIFY (VMDecommit (g_pprcNK, pKrnStk, cbStkSize - VM_PAGE_SIZE, VM_PAGER_AUTO, NULL));
    ...
    if (pcstk->dwPrcInfo) {
        ...
        // NOTE: VM might be switched inside kernel. We need to make sure we get back to the right VM 
        //       before accessing user mode TLS.
        if (pVMProc != pcstk->pprcVM) {
            SwitchVM (pcstk->pprcVM);
        }
