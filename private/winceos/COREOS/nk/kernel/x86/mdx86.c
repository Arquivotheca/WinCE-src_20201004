...
void SafeIdentifyCpu(void)
{
    __asm {
        ...
        cpuid
        mov     edi, ecx                // store result from cpuid into edi before re-using ecx
        mov     ebx, eax
        mov     ecx, eax
        shr     ebx, 4
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
DWORD GetIlockRestartAddr (DWORD dwPc)
{
    if (dwPc > (DWORD)g_pfnKrnExpInterlockedPopEntrySListResume
     && dwPc < (DWORD)g_pfnKrnExpInterlockedPopEntrySListEnd) {
        return (DWORD)g_pfnKrnExpInterlockedPopEntrySListResume;
    } else if (dwPc > (DWORD)g_pfnExpInterlockedPopEntrySListResume
     && dwPc < (DWORD)g_pfnExpInterlockedPopEntrySListEnd) {
        return (DWORD)g_pfnExpInterlockedPopEntrySListResume;
    } else { 
        return 0;
    }
}
...
void MDCheckInterlockOperation (PCONTEXT pCtx)
{
    ...
    DWORD dwNewPc = GetIlockRestartAddr (dwPc);
    if (0 != dwNewPc){      // in interlocked region
        ...
        CONTEXT_TO_PROGRAM_COUNTER (pCtx) = dwNewPc;
    }
}
    ...
    *pdwExcpId = id;
    ...
    case ID_PAGE_FAULT:        // Page fault
        ...
        if (pExr->ExceptionInformation[0]           // write access
            && (addr >= VM_SHARED_HEAP_BASE)        // kernel writeable
            && !fInKMode) {                         // user mode
    ...
    ) 
{
    ...
    if (((lpContext->ContextFlags & CONTEXT_FLOATING_POINT) == CONTEXT_FLOATING_POINT)
        && !InPrivilegeCall ()) {
        // need to be in ring-0 to execute 'clts' instruction
        fRet = KCall ((PKFN) SCHL_DoThreadGetContext, pth, lpContext);
