...
BOOL DbgrDeInitDebuggeeProcess(PPROCESS pprc)
{
    ...
    // debugger detached, clear the "BP might be set" flag.
    ClearProcessFlags (pprc, PROC_BP_MIGHT_BE_SET);
    ...
    do {
        ...
        if (pprc) {
            ...
            if (fTerminate) {
                ...
                DoWaitForObjects (1, &pprc->phdProcEvt, 3000);
            }
        }
        UnlockHandleData(phd);
    ...
    ) 
{
    ...
    if (bSecond && (er->ExceptionCode != STATUS_BREAKPOINT) && pDebugger && !pprc->pDbgrThrd) {
        CE_PROCESS_INFORMATION CeProcInfo = {sizeof(CE_PROCESS_INFORMATION)};
        DWORD ExitCode;
        WCHAR cmdline[11];
        cmdline[0] = '0';
        cmdline[1] = 'x';
    ...
    if (pprc->pDbgrThrd) {
        ...
        // capture FPU context before notifying user debugger
