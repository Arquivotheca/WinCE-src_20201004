...
const PFNVOID Win32ExtMethods[] = {
    ...
    (PFNVOID)NotSupported,                  // 111 - Unsupported
    (PFNVOID)NotSupported,                  // 112 - Unsupported
    ...
    (PFNVOID)NKRegisterOOMNotification,     // 136 - RegisterOOMNotification
    (PFNVOID)NKIsSystemProcess,             // 137 - IsSystemProcess
    (PFNVOID)NotSupported,                  // 138 - SignalNoAppToKill
...
const PFNVOID Win32IntMethods[] = {
    ...
    (PFNVOID)InputDebugCharW,               // 76 - InputDebugCharW
    (PFNVOID)NotSupported,                  // 77 - SetLowestScheduledPriority
    (PFNVOID)NKUpdateNLSInfo,               // 78 - UpdateNLSInfoEx
...
static const ULONGLONG win32Sigs [] = {
    ...
    FNSIG3 (DW, IO_PDW, IO_PDW),                // 111 - Unsupported
    FNSIG1 (DW),                                // 112 - Unsupported
    ...
    FNSIG3 (O_PDW, O_PDW, O_PDW),               // 136 - RegisterOOMNotification
    FNSIG1 (DW),                                // 137 - IsSystemProcess
    FNSIG0 (),                                  // 138 - SignalNoAppToKill
...
void NKPowerOffSystem (void) 
{
    ...
    /* Tell GWES and device manager that we are turning power off */
    ...
    case eSnapPass2:
        if (g_cTotalDirty) {
            NKDbgPrintfW (L"ERROR! There are %d dirty pages unflushed while taking snapshot, operation aborted!", g_cTotalDirty);
            break;
        }

        // atomic map files are registry and database volumes. 
        // If it's r/w, we need to assume that it can grow and thus need to
        // re-verify when booted from snapshot. keeping those pages around
        // are not useful as we'll need to discard them on boot anyway.
        FlushAllMapFiles (TRUE);
