...
/* Convert the file name into a ULONGLONG. */
ULONGLONG DirectoryHash_t::HashFileName(__in_z LPCWSTR pFileName, const BOOL strictlyAscii)
{
    ULONGLONG ullHash = 0;
    size_t Length = 0;
    ...
    for( size_t x = 0; x < Length; x++ )
    {
        ...
        if(strictlyAscii && IsNonAsciiChar( pBytes ) )
        {
            ullHash = 0;
            goto exit_computehash;
        }
...
BOOL DirectoryHash_t::HashVolume()
{
    ...
    if( InterlockedCompareExchangePointer( (LPVOID*)&m_pHashMaps, pHashMap, NULL ) == NULL )
    {
...
/* Hash all the paths specified for this volume. Only "\Windows" for now. */
BOOL DirectoryHash_t::ReHashVolume ()
{
    BOOL fResult = TRUE;
    ...
    if (m_pHashMaps) {
        ...
        HashMap_t* pHashMap = NULL;
        fResult = CreateHashMap (&pHashMap, L"\\Windows")
               && HashPath (pHashMap);

        if (fResult) {
            // swap the existing one with the new one we created.
            pHashMap = (HashMap_t*) InterlockedExchangePointer ((LPVOID*) &m_pHashMaps, pHashMap);
...
BOOL DirectoryHash_t::FileMightExist( LPCWSTR pFilePath, DWORD* pdwError )
{
    ...
    for( DWORD x = 0; x < dwTokens; x++ )
    {
        ...
        if( pMap )
        {
            ...
            ULONGLONG ullHash = HashFileName( pFile , TRUE );
...
{
    ULONGLONG ullHash = HashFileName( pFileName, TRUE);
