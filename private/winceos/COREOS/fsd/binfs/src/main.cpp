...
void DeleteVolume(BinVolume *pVolume)
{
    ...
    if (INVALID_CACHE_ID != pVolume->dwCacheId) {
        FSDMGR_DeleteCache (pVolume->dwCacheId);
    }
    if (pVolume->pReadBuf)
...
BinVolume *CreateVolume(HDSK hDsk)
{
    ...
    if (pVolume) {
        memset (pVolume, 0, sizeof (BinVolume));
        pVolume->hDsk = hDsk;
        InitializeCriticalSection(&pVolume->csComp);
        pVolume->dwCacheId = INVALID_CACHE_ID;
        ...
        if (!cbSector || (cbSector & (cbSector - 1))) {
            // sector size 0 (IOCTL failed??) or not power of 2
            ERRORMSG (1, (L"CreateVolume: Invalid Sector size %d, volume not created\r\n", cbSector));
...
BOOL ReadDirEntries(BinVolume *pVolume, DWORD dwCurRegion, DWORD dwRomHdrOffset)
{
    ...
    if (!ReadAtOffset(pVolume, &romhdr, sizeof(ROMHDR), dwRomHdrOffset)) {
    ...
    if (dwNumModules) {
        ...
        if (pBuffer) {
            fRet = ReadAtOffset(pVolume, pBuffer, sizeof(TOCentry) * dwNumModules, dwRomHdrOffset + sizeof(ROMHDR));
            if (fRet ) {
                DWORD i, j;
                TOCentry* pte = (TOCentry*) pBuffer;
                for (i=0;i < dwNumModules; i++) {
                    DWORD dwFileNameLength = 0;
                    fRet = ReadAtOffset( pVolume, (PBYTE) szAscii,  MAX_PATH, AdjustOffset(pVolume, dwCurRegion, (DWORD) pte[i].lpszFileName));
                    j = 0;
                    ...
                    dwFileNameLength = wcslen(szFileName);
                    pTemp->szFileName = new WCHAR[dwFileNameLength+1];
                    if (pTemp->szFileName) {
                        VERIFY(!wcscpy_s( pTemp->szFileName, dwFileNameLength +1, szFileName));
                    }                    
                    pTemp->pNext = NULL;
                    pTemp->pNext = NULL;
                    pTemp->pe32 = new e32_rom;
                    if (pTemp->pe32) {
                        ReadAtOffset(pVolume, (PBYTE)pTemp->pe32, sizeof(e32_rom), AdjustOffset(pVolume, dwCurRegion, pte[i].ulE32Offset));
                        ...
                        if (pTemp->po32) {
                            ReadAtOffset( pVolume, (PBYTE)pTemp->po32, sizeof(o32_rom)*pTemp->pe32->e32_objcnt, AdjustOffset(pVolume, dwCurRegion, pte[i].ulO32Offset));
    ...
    if (dwNumFiles) {
        ...
        if (pBuffer) {
            fRet = ReadAtOffset(pVolume, pBuffer, sizeof(FILESentry) * dwNumFiles, dwRomHdrOffset+ sizeof(ROMHDR) + (sizeof(TOCentry) * dwNumModules));
            if (fRet) {
                FILESentry* pfe = (FILESentry*) pBuffer;
                DWORD i, j;
                
                for (i=0;i < dwNumFiles; i++) {
                    fRet = ReadAtOffset(pVolume, (PBYTE) szAscii,  MAX_PATH, AdjustOffset( pVolume, dwCurRegion , (DWORD) pfe[i].lpszFileName));
                    j = 0;
                    memset( szFileName, 0, MAX_PATH);
                    while( (j < MAX_PATH) && szAscii[j]) {
                        szFileName[j] = szAscii[j];
...
BOOL InitVolume(BinVolume *pVolume)
{
    ...
    pVolume->dwCacheId = FSDMGR_CreateCache (pVolume->hDsk, 
                                             0,
                                             pVolume->diskInfo.di_total_sectors - 1,
                                             0,
                                             pVolume->diskInfo.di_bytes_per_sect,
                                             CACHE_FLAG_DISABLE_LAZY_WRITER);

    for (dwRegion = 0; dwRegion < pVolume->dwNumRegions; dwRegion++) {
skipchain:        
        if (!ReadAtOffset(pVolume, &dwSignature, sizeof(DWORD), ROM_SIGNATURE_OFFSET+dwOffset)) {
            break;
        }    
        ...
        ReadAtOffset(pVolume, &dwHdrStart, sizeof(DWORD), ROM_SIGNATURE_OFFSET+sizeof(DWORD)+dwOffset);
...
BOOL RegisterVolume(BinVolume *pVolume)
{
    ...
    DWORD fDisableXIP = FALSE;
    DWORD cbRet;
    STORAGEDEVICEINFO sdi;
    if (!FSDMGR_GetRegistryString(pVolume->hDsk, L"Folder", szFolderName, MAX_PATH)) {
        VERIFY(!wcscpy_s( szFolderName, _countof(szFolderName), DEFAULT_VOLUME_NAME));
    }
    FSDMGR_GetRegistryValue(pVolume->hDsk, L"DisableXIP", &fDisableXIP);
    
    if (!fDisableXIP
        && (VM_PAGE_SIZE == pVolume->diskInfo.di_bytes_per_sect)
