                            ...
                            LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreate, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
    ...
    if (ERROR_SUCCESS == dwError) {
        ...
        if (wcslen(lpFileName) > SYSTEM_DIR_LEN) {
            if ((_wcsnicmp( lpFileName, SYSTEM_DIR, SYSTEM_DIR_LEN) == 0) && ((*(lpFileName+SYSTEM_DIR_LEN) == L'\\') || (*(lpFileName+SYSTEM_DIR_LEN) == L'/'))){
        ...
        while(pDirectory) {
            if (_wcsicmp( pDirectory->szFileName, lpFileName) == 0) 
                break;
