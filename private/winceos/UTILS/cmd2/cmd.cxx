...
static int cmd_Initialize (void) {
...
#if defined (UNDER_CE)
    pCmdState->hCoreDll      = LoadLibrary (L"coredll.dll");
#endif
    ...
    if (WAIT_OBJECT_0 == WaitForAPIReady (SH_SHELL, 0)) {
        pCmdState->ShellExecuteEx = (SEE_t)GetProcAddress (pCmdState->hCoreDll, TEXT("ShellExecuteEx"));
    }
    ...
    if (! (GetStdioPathW(1, szStdPath, &dwLen) && dwLen && szStdPath[0])) {
        ...
        if(ferror(stdout)) {
            ...
            MessageBoxW_t pmb = (MessageBoxW_t)GetProcAddress (pCmdState->hCoreDll, L"MessageBoxW");
            if (pmb)
    ...
    CeGetConsoleRows((HANDLE)_get_osfhandle((int)_fileno(stdout)), &(pCmdState->yMax));
    CeGetConsoleCols((HANDLE)_get_osfhandle((int)_fileno(stdout)), &(pCmdState->xMax));
    ...
    if ( CeSetControlCEvent((HANDLE)_get_osfhandle((int)_fileno(stdout)), &g_hControlCEvent) ) {
    ...
    if (LoadStringW (pCmdState->hInstance, IDS_CMD_TITLE, szTitleBuf1, CMD_BUF_TINY) && (WideCharToMultiByte(CP_ACP, 0, szTitleBuf1, -1, szTitleBuf2, CMD_BUF_TINY, NULL, NULL) != 0)) {
        ...
        CeSetConsoleTitleA((HANDLE)_get_osfhandle((int)_fileno(gIO[CMDIO_OUT])), szTitleBuf2);
    }
#else
...
void cmd_Uninitialize()
{
    ...
    if (pCmdState) {
        cmdutil_Free(pCmdState->lpBufCmd);
        cmdutil_Free(pCmdState->lpBufInput);
        cmdutil_ReleaseFMem(pCmdState->pListMem);
        FreeLibrary(pCmdState->hCoreDll);
        cmdutil_Free(pCmdState);
    }        
}
