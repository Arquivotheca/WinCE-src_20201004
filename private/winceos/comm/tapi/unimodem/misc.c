    ...
    )
{
    ...
    if( NULL == lpszDeviceName ) {
        ...
        if (wcscpy_s( ptLineDev->szDeviceName, _countof(ptLineDev->szDeviceName), lpszDeviceName )) {
            DEBUGMSG(ZONE_ERROR|ZONE_INIT,
                 (TEXT("UNIMODEM:createLineDev - wcscpy_s failed for DeviceName.\r\n")));
            goto abort_registry_opened;
        }
    }
    ...
    if( dwRetCode != ERROR_SUCCESS ) { // Friendly not found, use something else
        ...
        if (wcscpy_s( ptLineDev->szFriendlyName, _countof(ptLineDev->szFriendlyName), ptLineDev->szDeviceName)) {
            DEBUGMSG(ZONE_ERROR|ZONE_INIT,
                 (TEXT("UNIMODEM:createLineDev - wcscpy_s failed for FriendlyName.\r\n")));
            goto abort_registry_opened;
        }
    }
...
)
{
    ...
    LoadString(TspiGlobals.hInstance, dwTitle, szWindowTitle, sizeof(szWindowTitle)/sizeof(WCHAR));
