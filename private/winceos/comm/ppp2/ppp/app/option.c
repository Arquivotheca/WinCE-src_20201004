         ...
         pOption = (POptionHeader)((PBYTE)pOption + pOption->Len))
    {
        ...
        if (pOption->Len > OPTION_HEADER_LENGTH)
        {
            ...
            *(pBuffer++) = '=';
            ...
            {
#ifdef UNDER_CE
                DWORD dwRemainingCchBuff = cchBuffer - (pBuffer - pBuffOrigin);
                pInfo->pDescriptor->OptionToStrCb(pContext->CbContext, pInfo, pOptData, cbOptData, &pBuffer, dwRemainingCchBuff);
#else
...
//          is to be sent.
{
    ...
    do
    {
        ...
        case PPP_CONFIGURE_REQUEST:
            ...
            if (dwResult == NO_ERROR && pcbResponse)
